...
#pragma omp simd reduction(+ : result) aligned(a, b : 32)
    ...
    float DistanceInnerProduct<T>::inner_product(const T *a, const T *b,
        ...
        if (!std::is_floating_point<T>::value) {
            TURBO_LOG(ERROR) << "ERROR: Inner Product only defined for float currently."
                       << std::endl;
...
#define SSE_DOT(addr1, addr2, dest, tmp1, tmp2) \
  ...
  dest = _mm128_add_ps(dest, tmp1);
    ...
    float DistanceFastL2<T>::norm(const T *a, unsigned size) const {
        if (!std::is_floating_point<T>::value) {
            TURBO_LOG(ERROR) << "ERROR: FastL2 only defined for float currently."
                       << std::endl;
            throw tann::ANNException(
...
#define AVX_DOT(addr1, addr2, dest, tmp1, tmp2) \
  ...
  dest = _mm256_add_ps(dest, tmp1);
    ...
    tann::Distance<float> *get_distance_function(tann::Metric m) {
        if (m == tann::Metric::L2) {
            if (Avx2SupportedCPU) {
                TURBO_LOG(INFO) << "L2: Using AVX2 distance computation DistanceL2Float"
                                << std::endl;
                ...
                TURBO_LOG(INFO) << "L2: Older CPU. Using slow distance computation"
                                << std::endl;
                return new tann::SlowDistanceL2Float();
            }
        } else if (m == tann::Metric::COSINE) {
            TURBO_LOG(INFO) << "Cosine: Using either AVX or AVX2 implementation"
                            << std::endl;
            return new tann::DistanceCosineFloat();
        } else if (m == tann::Metric::INNER_PRODUCT) {
            TURBO_LOG(INFO) << "Inner product: Using AVX2 implementation "
                               "AVXDistanceInnerProductFloat"
                            << std::endl;
            return new tann::AVXDistanceInnerProductFloat();
        } else if (m == tann::Metric::FAST_L2) {
            TURBO_LOG(INFO) << "Fast_L2: Using AVX2 implementation with norm "
    ...
    tann::Distance<int8_t> *get_distance_function(tann::Metric m) {
        if (m == tann::Metric::L2) {
            if (Avx2SupportedCPU) {
                TURBO_LOG(INFO) << "Using AVX2 distance computation DistanceL2Int8.";
                return new tann::DistanceL2Int8();
            } else if (AvxSupportedCPU) {
                TURBO_LOG(INFO) << "AVX2 not supported. Using AVX distance computation";
                return new tann::AVXDistanceL2Int8();
            } else {
                TURBO_LOG(INFO) << "Older CPU. Using slow distance computation "
                                   "SlowDistanceL2Int<int8_t>.";
                return new tann::SlowDistanceL2Int<int8_t>();
            }
        } else if (m == tann::Metric::COSINE) {
            TURBO_LOG(INFO) << "Using either AVX or AVX2 for Cosine similarity "
                               "DistanceCosineInt8.";
