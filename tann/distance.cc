...
#pragma omp simd reduction(+ : result) aligned(a, b : 32)
  ...
  float DistanceInnerProduct<T>::inner_product(const T *a, const T *b,
    ...
    if (!std::is_floating_point<T>::value) {
      tann::cerr << "ERROR: Inner Product only defined for float currently."
                    << std::endl;
...
#define SSE_DOT(addr1, addr2, dest, tmp1, tmp2) \
  ...
  float DistanceFastL2<T>::norm(const T *a, unsigned size) const {
    if (!std::is_floating_point<T>::value) {
      tann::cerr << "ERROR: FastL2 only defined for float currently."
                    << std::endl;
      throw tann::ANNException(
...
#define AVX_DOT(addr1, addr2, dest, tmp1, tmp2) \
  ...
  tann::Distance<float> *get_distance_function(tann::Metric m) {
    if (m == tann::Metric::L2) {
      if (Avx2SupportedCPU) {
        tann::cout << "L2: Using AVX2 distance computation DistanceL2Float"
                      << std::endl;
        ...
        tann::cout << "L2: Older CPU. Using slow distance computation"
                      << std::endl;
        return new tann::SlowDistanceL2Float();
      }
    } else if (m == tann::Metric::COSINE) {
      tann::cout << "Cosine: Using either AVX or AVX2 implementation"
                    << std::endl;
  ...
  tann::Distance<int8_t> *get_distance_function(tann::Metric m) {
    if (m == tann::Metric::L2) {
      if (Avx2SupportedCPU) {
        tann::cout << "Using AVX2 distance computation DistanceL2Int8."
                      << std::endl;
        return new tann::DistanceL2Int8();
      } else if (AvxSupportedCPU) {
        tann::cout << "AVX2 not supported. Using AVX distance computation"
                      << std::endl;
