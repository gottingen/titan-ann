...
template <typename T> tann::Metric Distance<T>::get_metric() const
{
...
{
#ifdef _WINDOWS
    return tann::CosineSimilarity2<int8_t>(a, b, length);
#else
...
{
#ifdef _WINDOWS
    return tann::CosineSimilarity2<float>(a, b, length);
#else
...
template <typename T> float DistanceInnerProduct<T>::inner_product(const T *a, const T *b, uint32_t size) const
{
    ...
    {
        tann::cerr << "ERROR: Inner Product only defined for float currently." << std::endl;
        throw tann::ANNException("ERROR: Inner Product only defined for float currently.", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }
...
template <typename T> float DistanceFastL2<T>::norm(const T *a, uint32_t size) const
{
    if (!std::is_floating_point<T>::value)
    {
        tann::cerr << "ERROR: FastL2 only defined for float currently." << std::endl;
        throw tann::ANNException("ERROR: FastL2 only defined for float currently.", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }
    float result = 0;
#ifdef __GNUC__
...
template <> tann::Distance<float> *get_distance_function(tann::Metric m)
{
    if (m == tann::Metric::L2)
    {
        if (Avx2SupportedCPU)
        {
            tann::cout << "L2: Using AVX2 distance computation DistanceL2Float" << std::endl;
            return new tann::DistanceL2Float();
        }
        else if (AvxSupportedCPU)
        {
            tann::cout << "L2: AVX2 not supported. Using AVX distance computation" << std::endl;
            return new tann::AVXDistanceL2Float();
        }
        else
        {
            tann::cout << "L2: Older CPU. Using slow distance computation" << std::endl;
            return new tann::SlowDistanceL2<float>();
        }
    }
    else if (m == tann::Metric::COSINE)
    {
        tann::cout << "Cosine: Using either AVX or AVX2 implementation" << std::endl;
        return new tann::DistanceCosineFloat();
    }
    else if (m == tann::Metric::INNER_PRODUCT)
    {
        tann::cout << "Inner product: Using AVX2 implementation "
                         "AVXDistanceInnerProductFloat"
                      << std::endl;
        return new tann::AVXDistanceInnerProductFloat();
    }
    else if (m == tann::Metric::FAST_L2)
    {
        tann::cout << "Fast_L2: Using AVX2 implementation with norm "
                         "memoization DistanceFastL2<float>"
                      << std::endl;
        return new tann::DistanceFastL2<float>();
    }
    else
    {
...
template <> tann::Distance<int8_t> *get_distance_function(tann::Metric m)
{
    if (m == tann::Metric::L2)
    {
        if (Avx2SupportedCPU)
        {
            tann::cout << "Using AVX2 distance computation DistanceL2Int8." << std::endl;
            return new tann::DistanceL2Int8();
        }
        else if (AvxSupportedCPU)
        {
            tann::cout << "AVX2 not supported. Using AVX distance computation" << std::endl;
            return new tann::AVXDistanceL2Int8();
        }
        else
        {
            tann::cout << "Older CPU. Using slow distance computation "
                             "SlowDistanceL2Int<int8_t>."
                          << std::endl;
            return new tann::SlowDistanceL2<int8_t>();
        }
    }
    else if (m == tann::Metric::COSINE)
    {
        tann::cout << "Using either AVX or AVX2 for Cosine similarity "
                         "DistanceCosineInt8."
                      << std::endl;
        return new tann::DistanceCosineInt8();
    }
    else
    {
        std::stringstream stream;
...
template <> tann::Distance<uint8_t> *get_distance_function(tann::Metric m)
{
    if (m == tann::Metric::L2)
    {
#ifdef _WINDOWS
        tann::cout << "WARNING: AVX/AVX2 distance function not defined for Uint8. "
                         "Using "
                         "slow version. "
                         "Contact gopalsr@microsoft.com if you need AVX/AVX2 support."
                      << std::endl;
#endif
        return new tann::DistanceL2UInt8();
    }
    else if (m == tann::Metric::COSINE)
    {
        tann::cout << "AVX/AVX2 distance function not defined for Uint8. Using "
                         "slow version SlowDistanceCosineUint8() "
                         "Contact gopalsr@microsoft.com if you need AVX/AVX2 support."
                      << std::endl;
        return new tann::SlowDistanceCosineUInt8();
    }
    else
    {
        std::stringstream stream;
...
template TURBO_DLL class SlowDistanceL2<float>;
template TURBO_DLL class SlowDistanceL2<int8_t>;
template TURBO_DLL class SlowDistanceL2<uint8_t>;
