...
namespace tann {
  ...
  void FixedChunkPQTable::load_pq_centroid_bin(const char* pq_table_file,
    ...
    _u64* file_offset_data;  // since load_bin only sets the pointer, no need
                             // to delete.
    tann::load_bin<_u64>(files, pq_table_file, file_offset_data, nr, nc);
#else
    std::unique_ptr<_u64[]> file_offset_data;
    tann::load_bin<_u64>(pq_table_file, file_offset_data, nr, nc);
#endif
    ...
    tann::load_bin<float>(files, pq_table_file, tables, nr, nc,
                             file_offset_data[0]);
#else
    tann::load_bin<float>(pq_table_file, tables, nr, nc,
                             file_offset_data[0]);
#endif
...
#ifdef EXEC_ENV_OLS
    tann::load_bin<float>(files, pq_table_file, centroid, nr, nc,
                             file_offset_data[1]);
#else
    tann::load_bin<float>(pq_table_file, centroid, nr, nc,
                             file_offset_data[1]);
#endif
...
#ifdef EXEC_ENV_OLS
    tann::load_bin<uint32_t>(files, pq_table_file, chunk_offsets, nr, nc,
                                file_offset_data[chunk_offsets_index]);
#else
    tann::load_bin<uint32_t>(pq_table_file, chunk_offsets, nr, nc,
                                file_offset_data[chunk_offsets_index]);
#endif
...
#pragma omp parallel for schedule(static, 65536)
    ...
    cumul_bytes[1] =
        cumul_bytes[0] +
        tann::save_bin<float>(pq_pivots_path.c_str(), full_pivot_data.get(),
                                 (size_t) num_centers, dim, cumul_bytes[0]);
    cumul_bytes[2] =
        cumul_bytes[1] + tann::save_bin<float>(pq_pivots_path.c_str(),
                                                  centroid.get(), (size_t) dim,
                                                  1, cumul_bytes[1]);
    cumul_bytes[3] =
        cumul_bytes[2] + tann::save_bin<uint32_t>(
...
#pragma omp parallel for schedule(static, 65536)
    ...
    cumul_bytes[1] =
        cumul_bytes[0] +
        tann::save_bin<float>(opq_pivots_path.c_str(), full_pivot_data.get(),
                                 (size_t) num_centers, dim, cumul_bytes[0]);
    cumul_bytes[2] =
        cumul_bytes[1] + tann::save_bin<float>(opq_pivots_path.c_str(),
                                                  centroid.get(), (size_t) dim,
                                                  1, cumul_bytes[1]);
    cumul_bytes[3] =
        cumul_bytes[2] + tann::save_bin<uint32_t>(
  ...
  int generate_pq_data_from_pivots(const std::string data_file,
    ...
    if (!file_exists(pq_pivots_path)) {
      ...
      tann::load_bin<_u64>(pq_pivots_path.c_str(), file_offset_data, nr, nc,
                              0);
      ...
      tann::load_bin<float>(pq_pivots_path.c_str(), full_pivot_data, nr, nc,
                               file_offset_data[0]);
      ...
      tann::load_bin<float>(pq_pivots_path.c_str(), centroid, nr, nc,
                               file_offset_data[1]);
      ...
      tann::load_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets, nr, nc,
                                  file_offset_data[2]);
...
#pragma omp parallel for schedule(static, 8192)
      ...
      if (num_centers > 256) {
        ...
        tann::convert_types<uint32_t, uint8_t>(block_compressed_base.get(),
