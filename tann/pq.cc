...
void FixedChunkPQTable::load_pq_centroid_bin(const char *pq_table_file, size_t num_chunks)
{
    ...
    if (nr != 4 && nr != 5)
    {
        tann::cout << "Error reading pq_pivots file " << pq_table_file
                      ...
                      << " or " << 5;
        throw tann::ANNException("Error reading pq_pivots file at offsets data.", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }

    if (nr == 4)
    {
        tann::cout << "Offsets: " << file_offset_data[0] << " " << file_offset_data[1] << " " << file_offset_data[2]
    ...
    if ((nr != this->ndims) || (nc != 1))
    {
        ...
        throw tann::ANNException("Error reading pq_pivots file at centroid data.", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }
    ...
    if (nc != 1 || (nr != num_chunks + 1 && num_chunks != 0))
    {
        ...
        throw tann::ANNException("Error loading chunk offsets file", -1, __FUNCSIG__, __FILE__, __LINE__);
    }
                       ...
                       bool make_zero_mean)
{
    ...
    if (file_exists(pq_pivots_path))
    {
        ...
        {
            tann::cout << "PQ pivot file exists. Not generating again" << std::endl;
            return -1;
        }
    }
    ...
    for (size_t i = 0; i < num_pq_chunks; i++)
    {
        ...
        tann::cout << "Processing chunk " << i << " with dimensions [" << chunk_offsets[i] << ", "
...
#pragma omp parallel for schedule(static, 65536)
    ...
    cumul_bytes[1] = cumul_bytes[0] + tann::save_bin<float>(pq_pivots_path.c_str(), full_pivot_data.get(),
                                                               (size_t)num_centers, dim, cumul_bytes[0]);
    cumul_bytes[2] = cumul_bytes[1] +
                     tann::save_bin<float>(pq_pivots_path.c_str(), centroid.get(), (size_t)dim, 1, cumul_bytes[1]);
    cumul_bytes[3] = cumul_bytes[2] + tann::save_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets.data(),
                                                                  chunk_offsets.size(), 1, cumul_bytes[2]);
    tann::save_bin<size_t>(pq_pivots_path.c_str(), cumul_bytes.data(), cumul_bytes.size(), 1, 0);

    tann::cout << "Saved pq pivot data to " << pq_pivots_path << " of size " << cumul_bytes[cumul_bytes.size() - 1]
                  << "B." << std::endl;
...
#pragma omp parallel for schedule(static, 65536)
    ...
    std::vector<size_t> cumul_bytes(4, 0);
    cumul_bytes[0] = METADATA_SIZE;
    cumul_bytes[1] = cumul_bytes[0] + tann::save_bin<float>(opq_pivots_path.c_str(), full_pivot_data.get(),
                                                               (size_t)num_centers, dim, cumul_bytes[0]);
    cumul_bytes[2] = cumul_bytes[1] +
                     tann::save_bin<float>(opq_pivots_path.c_str(), centroid.get(), (size_t)dim, 1, cumul_bytes[1]);
    cumul_bytes[3] = cumul_bytes[2] + tann::save_bin<uint32_t>(opq_pivots_path.c_str(), chunk_offsets.data(),
                                                                  chunk_offsets.size(), 1, cumul_bytes[2]);
    tann::save_bin<size_t>(opq_pivots_path.c_str(), cumul_bytes.data(), cumul_bytes.size(), 1, 0);

    tann::cout << "Saved opq pivot data to " << opq_pivots_path << " of size " << cumul_bytes[cumul_bytes.size() - 1]
                  << "B." << std::endl;

    std::string rotmat_path = opq_pivots_path + "_rotation_matrix.bin";
    tann::save_bin<float>(rotmat_path.c_str(), rotmat_tr.get(), dim, dim);
                                 ...
                                 bool use_opq)
{
    ...
    else
    {
        ...
        tann::load_bin<size_t>(pq_pivots_path.c_str(), file_offset_data, nr, nc, 0);
        ...
        tann::load_bin<float>(pq_pivots_path.c_str(), full_pivot_data, nr, nc, file_offset_data[0]);

        if ((nr != num_centers) || (nc != dim))
        {
            tann::cout << "Error reading pq_pivots file " << pq_pivots_path << ". file_num_centers  = " << nr
        ...
        tann::load_bin<float>(pq_pivots_path.c_str(), centroid, nr, nc, file_offset_data[1]);

        if ((nr != dim) || (nc != 1))
        {
            tann::cout << "Error reading pq_pivots file " << pq_pivots_path << ". file_dim  = " << nr
                          << ", file_cols = " << nc << " but expecting " << dim << " entries in 1 dimension.";
            throw tann::ANNException("Error reading pq_pivots file at centroid data.", -1, __FUNCSIG__, __FILE__,
                                        __LINE__);
        }

        tann::load_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets, nr, nc, file_offset_data[2]);

        if (nr != (uint64_t)num_pq_chunks + 1 || nc != 1)
        {
            tann::cout << "Error reading pq_pivots file at chunk offsets; file has nr=" << nr << ",nc=" << nc
                          << ", expecting nr=" << num_pq_chunks + 1 << ", nc=1." << std::endl;
            throw tann::ANNException("Error reading pq_pivots file at chunk offsets.", -1, __FUNCSIG__, __FILE__,
                                        __LINE__);
        }
        ...
        if (use_opq)
        {
            ...
            if (nr != (uint64_t)dim || nc != dim)
            {
                ...
                throw tann::ANNException("Error reading rotation matrix file.", -1, __FUNCSIG__, __FILE__, __LINE__);
            }
        }
    ...
    for (size_t block = 0; block < num_blocks; block++)
    {
        ...
        tann::convert_types<T, float>(block_data_T.get(), block_data_tmp.get(), cur_blk_size, dim);

        tann::cout << "Processing points  [" << start_id << ", " << end_id << ").." << std::flush;
...
#pragma omp parallel for schedule(static, 8192)
        ...
        else
        {
            ...
            tann::convert_types<uint32_t, uint8_t>(block_compressed_base.get(), pVec.get(), cur_blk_size,
                                                      num_pq_chunks);
