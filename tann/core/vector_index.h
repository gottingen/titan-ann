...
#ifndef TANN_CORE_VECTOR_INDEX_H_
#define TANN_CORE_VECTOR_INDEX_H_

#include "tann/utility/common.h"
#include "tann/core/search_query.h"
#include "tann/vector/vector_set.h"
#include "tann/vector/metadata_set.h"
#include "tann/utility/SimpleIniReader.h"
...
namespace tann {
    ...
    public:
        ...
        BuildIndex(const void *p_data, SizeType p_vectorNum, DimensionType p_dimension, bool p_normalized = false,
                   bool p_shareOwnership = false) = 0;

        virtual ErrorCode AddIndex(const void *p_data, SizeType p_vectorNum, DimensionType p_dimension,
                                   std::shared_ptr<MetadataSet> p_metadataSet, bool p_withMetaIndex = false,
                                   bool p_normalized = false) = 0;
        ...
        virtual ErrorCode
        SearchIndexWithFilter(QueryResult &p_query, std::function<bool(const ByteArray &)> filterFunc, int maxCheck = 0,
                              bool p_searchDeleted = false) const = 0;
        ...
        virtual ErrorCode SearchIndex(const void *p_vector, int p_vectorCount, int p_neighborCount, bool p_withMeta,
                                      BasicResult *p_results) const;

        virtual void ApproximateRNG(std::shared_ptr<VectorSet> &fullVectors, std::unordered_set<SizeType> &exceptIDS,
        ...
        virtual ErrorCode
        SetParameter(const std::string &p_param, const std::string &p_value, const std::string &p_section = "Index");
        ...
        virtual void SetQuantizer(std::shared_ptr<tann::COMMON::IQuantizer> quantizer) = 0;

        virtual ErrorCode LoadQuantizer(std::string p_quantizerFile);

        virtual std::shared_ptr<tann::COMMON::IQuantizer> GetQuantizer() {
            return m_pQuantizer;
        }
        ...
        virtual ErrorCode QuantizeVector(const void *p_data, SizeType p_num, ByteArray p_out) {
            if (m_pQuantizer != nullptr && p_out.Length() >= m_pQuantizer->GetNumSubvectors() * (size_t) p_num) {
                for (int i = 0; i < p_num; i++)
                    m_pQuantizer->QuantizeVector(
                            ((std::uint8_t *) p_data) + i * (size_t) (m_pQuantizer->ReconstructSize()),
                            p_out.Data() + i * (size_t) (m_pQuantizer->GetNumSubvectors()), false);
        ...
        virtual ErrorCode ReconstructVector(const void *p_data, SizeType p_num, ByteArray p_out) {
            if (m_pQuantizer != nullptr && p_out.Length() >= m_pQuantizer->ReconstructSize() * (size_t) p_num) {
                for (int i = 0; i < p_num; i++)
                    m_pQuantizer->ReconstructVector(
                            ((std::uint8_t *) p_data) + i * (size_t) (m_pQuantizer->GetNumSubvectors()),
                            p_out.Data() + i * (size_t) (m_pQuantizer->ReconstructSize()));
                return ErrorCode::Success;
            }
            return ErrorCode::Fail;
        }
        ...
        static ErrorCode LoadIndex(const std::string &p_config, const std::vector<ByteArray> &p_indexBlobs,
                                   std::shared_ptr<VectorIndex> &p_vectorIndex);

        static std::uint64_t
        EstimatedVectorCount(std::uint64_t p_memory, DimensionType p_dimension, VectorValueType p_valuetype,
                             SizeType p_vectorsInBlock, SizeType p_maxmeta, IndexAlgoType p_algo, int p_treeNumber,
                             int p_neighborhoodSize);

        static std::uint64_t
        EstimatedMemoryUsage(std::uint64_t p_vectorCount, DimensionType p_dimension, VectorValueType p_valuetype,
                             SizeType p_vectorsInBlock, SizeType p_maxmeta, IndexAlgoType p_algo, int p_treeNumber,
                             int p_neighborhoodSize);
        ...
        virtual ErrorCode
        RefineIndex(const std::vector<std::shared_ptr<Helper::DiskIO>> &p_indexStreams, IAbortOperation *p_abort) = 0;

        virtual ErrorCode SetWorkSpaceFactory(
                std::unique_ptr<tann::COMMON::IWorkSpaceFactory<tann::COMMON::IWorkSpace>> up_workSpaceFactory) = 0;
    ...
    public:
        ...
        std::shared_ptr<tann::COMMON::IQuantizer> m_pQuantizer = nullptr;
    };


} // namespace tann

#endif // TANN_CORE_VECTOR_INDEX_H_
