...
#ifndef TANN_CORE_DATASET_H_
#define TANN_CORE_DATASET_H_
...
namespace tann {
    namespace COMMON {
        ...
        public:
            ...
            Dataset(SizeType rows_, DimensionType cols_, SizeType rowsInBlock_, SizeType capacity_, T *data_ = nullptr,
                    bool shareOwnership_ = true) {
            ...
            void Initialize(SizeType rows_, DimensionType cols_, SizeType rowsInBlock_, SizeType capacity_,
                            T *data_ = nullptr, bool shareOwnership_ = true) {
                rows = rows_;
                cols = cols_;
                data = data_;
            ...
            ErrorCode AddBatch(const T *pData, SizeType num) {
                ...
                while (written < num) {
                    ...
                    std::memcpy(incBlocks[curBlockIdx] + ((size_t) curBlockPos) * cols,
                                pData + ((size_t) written) * cols, ((size_t) toWrite) * cols * sizeof(T));
                    written += toWrite;
                }
                incRows += written;
            ...
            ErrorCode Save(std::string sDataPointsFileName) const {
                TLOG_INFO("Save {} To {}", name.c_str(), sDataPointsFileName.c_str());
                auto ptr = f_createIO();
                if (ptr == nullptr ||
                    !ptr->Initialize(sDataPointsFileName.c_str(), std::ios::binary | std::ios::out))
            ...
            ErrorCode Load(std::string sDataPointsFileName, SizeType blockSize, SizeType capacity) {
                ...
                auto ptr = f_createIO();
                if (ptr == nullptr ||
                    !ptr->Initialize(sDataPointsFileName.c_str(), std::ios::binary | std::ios::in))
            ...
            ErrorCode Refine(const std::vector<SizeType> &indices, Dataset<T> &p_data) const {
                ...
                p_data.Initialize(R, cols, rowsInBlock + 1,
                                  static_cast<SizeType>(incBlocks.capacity() * (rowsInBlock + 1)));
