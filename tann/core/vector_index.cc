...
#include "tann/graph/kdt/index.h"
#include "tann/graph/bkt/index.h"
#include "tann/graph/spann/index.h"

typedef typename tann::Helper::Concurrent::ConcurrentMap<std::string, tann::SizeType> MetadataMap;

using namespace tann;

std::mt19937 tann::rg;

std::shared_ptr<Helper::DiskIO>(*tann::f_createIO)() = []() -> std::shared_ptr<Helper::DiskIO> { return std::shared_ptr<Helper::DiskIO>(new Helper::SimpleFileIO()); };
...
namespace tann {
    ...
    bool copyfile(const char* oldpath, const char* newpath) {
        ...
        {
            TLOG_ERROR("Unable to open files: %s %s\n", oldpath, newpath);
            return false;
        }
        ...
        while (readSize != 0) {
            if (output->WriteBinary(readSize, bufferHolder.get()) != readSize) {
                TLOG_ERROR("Unable to write file: %s\n", newpath);
                return false;
            }
...
VectorIndex::SaveIndex(const std::string& p_folderPath)
{
    ...
    ErrorCode ret = ErrorCode::Success;
    {
        auto configFile = tann::f_createIO();
...
{
    TLOG_INFO("Begin build index...\n");

    bool valueMatches = p_vectorSet->GetValueType() == GetVectorValueType();
    bool quantizerMatches = ((bool)m_pQuantizer) && (p_vectorSet->GetValueType() == tann::VectorValueType::UInt8);
...
VectorIndex::LoadQuantizer(std::string p_quantizerFile)
{
    ...
    {
        TLOG_ERROR("Failed to read quantizer file.\n");
        return ErrorCode::FailedOpenFile;
    }
    SetQuantizer(tann::COMMON::IQuantizer::LoadIQuantizer(ptr));
    if (!m_pQuantizer)
    {
        TLOG_ERROR("Failed to load quantizer.\n");
...
VectorIndex::LoadIndex(const std::string& p_loaderFilePath, std::shared_ptr<VectorIndex>& p_vectorIndex)
{
    ...
    if (iniReader.DoesSectionExist("MetaData"))
    {
        ...
        {
            TLOG_ERROR("Error: Failed to load metadata.\n");
            return ErrorCode::Fail;
        }
    ...
    if (iniReader.DoesSectionExist("Quantizer")) {
        p_vectorIndex->SetQuantizer(tann::COMMON::IQuantizer::LoadIQuantizer(handles[metaStart]));
...
VectorIndex::LoadIndexFromFile(const std::string& p_file, std::shared_ptr<VectorIndex>& p_vectorIndex)
{
    ...
    tann::Helper::IniReader iniReader;
    {
        ...
        if (tann::ErrorCode::Success != iniReader.LoadIni(bufferhandle)) return ErrorCode::FailedParseValue;
    }
    ...
    if (iniReader.DoesSectionExist("Quantizer"))
    {
        p_vectorIndex->SetQuantizer(tann::COMMON::IQuantizer::LoadIQuantizer(fp));
        if (!p_vectorIndex->m_pQuantizer) return ErrorCode::FailedParseValue;
    }
...
VectorIndex::LoadIndex(const std::string& p_config, const std::vector<ByteArray>& p_indexBlobs, std::shared_ptr<VectorIndex>& p_vectorIndex)
{
    ...
    if (tann::ErrorCode::Success != iniReader.LoadIni(fp)) return ErrorCode::FailedParseValue;
...
#include "tann/core/Common/cuda/TailNeighbors.hxx"

void VectorIndex::SortSelections(std::vector<Edge>* selections) {
  TLOG_DEBUG("Starting sort of final input on GPU");
...
void VectorIndex::ApproximateRNG(std::shared_ptr<VectorSet>& fullVectors, std::unordered_set<SizeType>& exceptIDS, int candidateNum, Edge* selections, int replicaCount, int numThreads, int numTrees, int leafSize, float RNGFactor, int numGPUs)
{
    ...
    TLOG_INFO("Starting GPU SSD Index build stage...\n");

    int metric = (GetDistCalcMethod() == tann::DistCalcMethod::Cosine);
...
#undef DefineVectorValueType
                    default:
                        TLOG_ERROR("Unable to get quantizer reconstruct type %s", Helper::Convert::ConvertToString<VectorValueType>(m_pQuantizer->GetReconstructType()));
                        }
                    }
                    else
                    {
    ...
    TLOG_INFO("Searching replicas ended. RNG failed count: %llu\n", static_cast<uint64_t>(rngFailedCountTotal.load()));
}
#endif
