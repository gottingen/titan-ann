...
#include "tann/graph/kdt/ParameterDefinitionList.h"
#undef DefineKDTParameter
            return ErrorCode::Success;
        }

        template <>
        void Index<std::uint8_t>::SetQuantizer(std::shared_ptr<tann::COMMON::IQuantizer> quantizer)
        {
        ...
        void Index<T>::SetQuantizer(std::shared_ptr<tann::COMMON::IQuantizer> quantizer)
        {
            ...
            if (quantizer)
            {
                TLOG_ERROR("Set non-null quantizer for index with data type other than BYTE");
            }
        }
        ...
        ErrorCode Index<T>::LoadIndexDataFromMemory(const std::vector<ByteArray>& p_indexBlobs)
        {
            ...
            {
                TLOG_ERROR("Index data is corrupted, please rebuild the index. Samples: {}, Graph: {}, DeletedID: {}.", m_pSamples.R(), m_pGraph.R(), m_deletedID.R());
...
#define DefineKDTParameter(VarName, VarType, DefaultValue, RepresentStr) \
    IOSTRING(p_configOut, WriteString, (RepresentStr + std::string("=") + GetParameter(RepresentStr) + std::string("")).c_str());
...
#undef DefineVectorValueType
        ...
        {
            TLOG_ERROR("Not Support Filter on KDT Index!");
...
#pragma omp parallel for
            ...
            TLOG_INFO("Build Tree time (s): {}", std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count());
            m_pGraph.BuildGraph<T>(this);
            auto t3 = std::chrono::high_resolution_clock::now();
            TLOG_INFO("Build Graph time (s): {}", std::chrono::duration_cast<std::chrono::seconds>(t3 - t2).count());
...
#undef DefineKDTParameter
            ...
            TLOG_INFO("Refine... from {} -> {}", GetNumSamples(), newR);
        ...
        ErrorCode Index<T>::AddIndex(const void* p_data, SizeType p_vectorNum, DimensionType p_dimension, std::shared_ptr<MetadataSet> p_metadataSet, bool p_withMetaIndex, bool p_normalized)
        {
            ...
            ErrorCode ret;
            {
                ...
                if (m_pSamples.AddBatch((const T*)p_data, p_vectorNum) != ErrorCode::Success ||
                    ...
                    TLOG_ERROR("Memory Error: Cannot alloc space for vectors!");
...
#define DefineKDTParameter(VarName, VarType, DefaultValue, RepresentStr) \
    else if (tann::Helper::StrUtils::StrEqualIgnoreCase(p_param, RepresentStr)) \
    { \
        TLOG_INFO("Setting {} with value {}", RepresentStr, p_value); \
        VarType tmp; \
        if (tann::Helper::Convert::ConvertStringTo<VarType>(p_value, tmp)) \
        { \
            VarName = tmp; \
        } \
    } \
...
#define DefineKDTParameter(VarName, VarType, DefaultValue, RepresentStr) \
    ...
    { \
        return tann::Helper::Convert::ConvertToString(VarName); \
    } \
...
#define DefineVectorValueType(Name, Type) \
template class tann::KDT::Index<Type>; \
