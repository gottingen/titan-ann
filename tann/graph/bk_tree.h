...
#ifndef TANN_GRAPH_BK_TREE_H_
#define TANN_GRAPH_BK_TREE_H_
    ...
    namespace COMMON
    {
        ...
        float RefineCenters(const Dataset<T>& data, KmeansArgs<T>& args)
        {
            ...
            if (maxcluster != -1 && (args.clusterIdx[maxcluster] < 0 || args.clusterIdx[maxcluster] >= data.R()))
                TLOG_DEBUG("maxcluster:{}({}) Error dist:{}", maxcluster, args.newCounts[maxcluster], args.clusterDist[maxcluster]);
...
#include "tann/core/Common/cuda/Kmeans.hxx"
        ...
        float TryClustering(const Dataset<T>& data,
            ...
            for (int iter = 0; iter < 100; iter++) {
                ...
                if (debug) {
                    ...
                    TLOG_INFO("iter {} dist:{} lambda:({},{}) counts:{}", iter, currDist, originalLambda, adjustedLambda, log.c_str());
                }
                */

                currDiff = RefineCenters<T, R>(data, args);
                //if (debug) TLOG_INFO("iter {} dist:{} diff:{}", iter, currDist, currDiff);
            ...
            if (debug) TLOG_INFO("Lambda:min({},{}) Max:{} Min:{} Avg:{} Std/Avg:{} Dist:{} NonZero/Total:{}/{}", originalLambda, adjustedLambda, maxCount, minCount, CountAvg, CountStd, currDist, availableClusters, args._DK);
...
#undef DefineVectorValueType
            ...
            */
            TLOG_INFO("Best Lambda Factor:{}", bestLambdaFactor);
...
#undef DefineVectorValueType
        ...
        public:
                ...
                bool dynamicK = false, IAbortOperation* abort = nullptr)
            {
                ...
                for (char i = 0; i < m_iTreeNumber; i++)
                {
                    ...
                    TLOG_INFO("Start to build BKTree {}", i + 1);
                    ...
                    TLOG_INFO("{} BKTree built, {} {}", i + 1, m_pTreeRoots.size() - m_pTreeStart[i], localindices.size());
                }
            }
            ...
            ErrorCode SaveTrees(std::shared_ptr<Helper::DiskIO> p_out) const
            {
                ...
                TLOG_INFO("Save BKT ({},{}) Finish!", m_iTreeNumber, treeNodeSize);
                return ErrorCode::Success;
            }

            ErrorCode SaveTrees(std::string sTreeFileName) const
            {
                TLOG_INFO("Save BKT to {}", sTreeFileName.c_str());
                auto ptr = f_createIO();
            ...
            ErrorCode LoadTrees(char* pBKTMemFile)
            {
                ...
                TLOG_INFO("Load BKT ({},{}) Finish!", m_iTreeNumber, treeNodeSize);
                return ErrorCode::Success;
            }
            ...
            {
                TLOG_INFO("Load BKT From {}", sTreeFileName.c_str());
                auto ptr = f_createIO();
        ...
        public:
            ...
            std::shared_ptr<tann::COMMON::IQuantizer> m_pQuantizer;
        };
    }
}
#endif  // TANN_GRAPH_BK_TREE_H_
