...
//
    ...
    template<typename T>
    tann::Metric Distance<T>::get_metric() const {
...
#pragma omp simd reduction(+ : result) aligned(a, b : 32)
    ...
    float DistanceInnerProduct<T>::inner_product(const T *a, const T *b, uint32_t size) const {
        if (!std::is_floating_point<T>::value) {
            ...
            throw tann::ANNException("ERROR: Inner Product only defined for float currently.", -1, __FUNCSIG__,
                                     __FILE__,
                                     __LINE__);
        }
...
// Get the right distance function for the given metric.
    template<>
    tann::Distance<float> *get_distance_function(tann::Metric m) {
        if (m == tann::Metric::L2) {
            if (Avx2SupportedCPU) {
    ...
    template<>
    tann::Distance<int8_t> *get_distance_function(tann::Metric m) {
        if (m == tann::Metric::L2) {
            if (Avx2SupportedCPU) {
    ...
    template<>
    tann::Distance<uint8_t> *get_distance_function(tann::Metric m) {
        if (m == tann::Metric::L2) {
#ifdef _WINDOWS
