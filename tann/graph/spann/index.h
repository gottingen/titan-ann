...
#include "tann/distance/SIMDUtils.h"
#include "tann/core/query_result_set.h"
#include "tann/graph/bk_tree.h"
#include "tann/core/work_space_pool.h"

#include "tann/core/labelset.h"
#include "tann/utility/SimpleIniReader.h"
#include "tann/utility/StringConvert.h"
#include "tann/concurrent/thread_pool.h"
...
namespace tann {
    ...
    namespace SPANN {
        ...
        private:
            ...
            int m_iBaseSquare;
            std::unique_ptr<tann::COMMON::IWorkSpaceFactory<ExtraWorkSpace>> m_workSpaceFactory;

        public:
            Index() {
                m_workSpaceFactory = std::make_unique<tann::COMMON::ThreadLocalWorkSpaceFactory<ExtraWorkSpace>>();
                m_fComputeDistance = std::function<float(const T *, const T *, DimensionType)>(
                        COMMON::DistanceCalcSelector<T>(m_options.m_distCalcMethod));
                m_iBaseSquare = (m_options.m_distCalcMethod == DistCalcMethod::Cosine) ? COMMON::Utils::GetBase<T>() *
            ...
            void SetQuantizer(std::shared_ptr<tann::COMMON::IQuantizer> quantizer);
            ...
            ErrorCode SearchIndexWithFilter(QueryResult &p_query, std::function<bool(const ByteArray &)> filterFunc,
            ...
            ErrorCode DebugSearchDiskIndex(QueryResult &p_query, int p_subInternalResultNum, int p_internalResultNum,
                                           SearchStats *p_stats = nullptr, std::set<int> *truth = nullptr,
            ...
            ErrorCode
            RefineSearchIndex(QueryResult &p_query, bool p_searchDeleted = false) const { return ErrorCode::Undefined; }

            ErrorCode SearchTree(QueryResult &p_query) const { return ErrorCode::Undefined; }

            ErrorCode AddIndex(const void *p_data, SizeType p_vectorNum, DimensionType p_dimension,
                               std::shared_ptr<MetadataSet> p_metadataSet, bool p_withMetaIndex = false,
                               bool p_normalized = false) { return ErrorCode::Undefined; }
            ...
            ErrorCode RefineIndex(const std::vector<std::shared_ptr<Helper::DiskIO>> &p_indexStreams,
                                  IAbortOperation *p_abort) { return ErrorCode::Undefined; }

            ErrorCode RefineIndex(std::shared_ptr<VectorIndex> &p_newIndex) { return ErrorCode::Undefined; }

            ErrorCode SetWorkSpaceFactory(
                    std::unique_ptr<tann::COMMON::IWorkSpaceFactory<tann::COMMON::IWorkSpace>> up_workSpaceFactory) {
                tann::COMMON::IWorkSpaceFactory<tann::COMMON::IWorkSpace> *raw_generic_ptr = up_workSpaceFactory.release();
                if (!raw_generic_ptr) return ErrorCode::Fail;


                tann::COMMON::IWorkSpaceFactory<ExtraWorkSpace> *raw_specialized_ptr = dynamic_cast<tann::COMMON::IWorkSpaceFactory<ExtraWorkSpace> *>(raw_generic_ptr);
                if (!raw_specialized_ptr) {
                    // If it is of type tann::COMMON::WorkSpace, we should pass on to child index
                    if (!m_index) {
                        delete raw_generic_ptr;
                        return ErrorCode::Fail;
                    } else {
                        return m_index->SetWorkSpaceFactory(
                                std::unique_ptr<tann::COMMON::IWorkSpaceFactory<tann::COMMON::IWorkSpace>>(
                                        raw_generic_ptr));
                    }

                } else {
                    m_workSpaceFactory = std::unique_ptr<tann::COMMON::IWorkSpaceFactory<ExtraWorkSpace>>(
                            raw_specialized_ptr);
        ...
        private:
            ...
            int SelectHeadDynamicallyInternal(const std::shared_ptr<COMMON::BKTree> p_tree, int p_nodeID,
