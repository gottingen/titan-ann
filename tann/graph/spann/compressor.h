    ...
    namespace SPANN
    {
        ...
        private:
            void CreateCDict()
            {
                ...
                if (cdict == NULL)
                {
                    TLOG_ERROR("ZSTD_createCDict() failed! ");
            ...
            void CreateDDict()
            {
                ...
                if (ddict == NULL)
                {
                    TLOG_ERROR("ZSTD_createDDict() failed! ");
            ...
            std::string CompressWithDict(const std::string &src)
            {
                ...
                if (cctx == NULL)
                {
                    TLOG_ERROR("ZSTD_createCCtx() failed! ");
                ...
                {
                    TLOG_ERROR("ZSTD compress error {}, ", ZSTD_getErrorName(compressed_size));
            ...
            std::size_t DecompressWithDict(const char* src, size_t srcSize, char* dst, size_t dstCapacity)
            {
                ...
                if (dctx == NULL)
                {
                    TLOG_ERROR("ZSTD_createDCtx() failed! ");
                ...
                {
                    TLOG_ERROR("ZSTD decompress error {}, ", ZSTD_getErrorName(decomp_size));
        ...
        public:
            ...
            std::size_t TrainDict(const std::string &samplesBuffer, const size_t *samplesSizes, unsigned nbSamples)
            {
                ...
                {
                    TLOG_ERROR("ZDICT_trainFromBuffer() failed: {} ", ZDICT_getErrorName(dictSize));
