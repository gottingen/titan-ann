...
#ifndef TANN_GRAPH_NEIGHBORHOOD_GRAPH_H_
#define TANN_GRAPH_NEIGHBORHOOD_GRAPH_H_

#include "tann/core/vector_index.h"

#include "tann/utility/common_utils.h"
#include "tann/core/dataset.h"
#include "tann/concurrent/fine_grained_lock.h"
...
namespace tann {
    namespace COMMON {
        ...
        public:
            ...
            virtual void
            InsertNeighbors(VectorIndex *index, const SizeType node, SizeType insertNode, float insertDist) = 0;
            ...
            virtual float GraphAccuracyEstimation(VectorIndex *index, const SizeType samples,
                                                  const std::unordered_map<SizeType, SizeType> *idmap = nullptr) {
...
#pragma omp parallel for schedule(dynamic)
            ...
            {
                SizeType initSize;
                tann::Helper::Convert::ConvertStringTo(index->GetParameter("NumberOfInitialDynamicPivots").c_str(), initSize);
            ...
            template<typename T>
            void PartitionByTptree(VectorIndex *index, std::vector<SizeType> &indices, const SizeType first,
                                   const SizeType last,
...
#undef DefineVectorValueType
            ...
            void PartitionByTptreeCore(VectorIndex *index, std::vector<SizeType> &indices, const SizeType first,
                                       const SizeType last,
                                       std::vector<std::pair<SizeType, SizeType>> &leaves) {
                if (last - first <= m_iTPTLeafSize) {
                    ...
                    SizeType splitidx = first;
                    {
                        ...
                        if (quantizer_exists) {
                            ...
                            indices_vectors.reset(new BasicVectorSet(ByteArray::Alloc(sizeof(R) * cols * count),
                                                                     GetEnumValueType<R>(), cols, count));
                            for (int i = 0; i < count; i++)
                                index->m_pQuantizer->ReconstructVector((uint8_t *) index->GetSample(indices[first + i]),
                                                                       indices_vectors->GetVector(i));
                        }

                        std::vector<float> Mean(cols, 0);
                        // calculate the mean of each dimension
                        for (SizeType j = first; j <= end; j++) {
                            R *v = (quantizer_exists) ? (R *) indices_vectors->GetVector(j - first)
                                                      : (R *) index->GetSample(indices[j]);
                        ...
                        for (int i = 0; i < iIteration; i++) {
                            ...
                            for (SizeType j = 0; j < count; j++) {
                                Val[j] = 0;
                                R *v = (quantizer_exists) ? (R *) indices_vectors->GetVector(j)
                                                          : (R *) index->GetSample(indices[first + j]);
                        ...
                        while (i <= j) {
                            float val = 0;
                            R *v = (quantizer_exists) ? (R *) indices_vectors->GetVector(i - first)
                                                      : (R *) index->GetSample(indices[i]);
                            for (int k = 0; k < m_numTopDimensionTPTSplit; k++) {
            ...
            void BuildInitKNNGraph(VectorIndex *index, const std::unordered_map<SizeType, SizeType> *idmap) {
                ...
                COMMON::Dataset<float> NeighborhoodDists(m_iGraphSize, m_iNeighborhoodSize, index->m_iDataBlockSize,
                                                         index->m_iDataCapacity);
                std::vector<std::vector<SizeType>> TptreeDataIndices(m_iTPTNumber, std::vector<SizeType>(m_iGraphSize));
                std::vector<std::vector<std::pair<SizeType, SizeType>>>
                TptreeLeafNodes(m_iTPTNumber, std::vector<std::pair<SizeType, SizeType>>());
                ...
                TLOG_INFO("Parallel TpTree Partition begin");
#pragma omp parallel for schedule(dynamic)
                for (int i = 0; i < m_iTPTNumber; i++) {
                    ...
                    TLOG_INFO("Finish Getting Leaves for Tree {}", i);
                }
                TLOG_INFO("Parallel TpTree Partition done");
                auto t2 = std::chrono::high_resolution_clock::now();
                TLOG_INFO("Build TPTree time (s): {}",
...
#pragma omp parallel for schedule(dynamic)
                ...
                TLOG_INFO("Process TPTree time (s): {}",
            ...
            void BuildGraph(VectorIndex *index, const std::unordered_map<SizeType, SizeType> *idmap = nullptr) {
                ...
                auto t2 = std::chrono::high_resolution_clock::now();
                TLOG_INFO("BuildInitKNNGraph time (s): {}",
                             std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count());

                RefineGraph<T>(index, idmap);

                auto t3 = std::chrono::high_resolution_clock::now();
                TLOG_INFO("BuildGraph time (s): {}",
...
#pragma omp parallel for schedule(dynamic)
                ...
                auto t1 = std::chrono::high_resolution_clock::now();
                TLOG_INFO("Calculate Indegree time (s): {}",
...
#pragma omp parallel for schedule(dynamic)
                ...
                auto t2 = std::chrono::high_resolution_clock::now();
                TLOG_INFO("Rebuild RNG time (s): {} Graph Acc: {}",
                             std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count(),
...
#pragma omp parallel for schedule(dynamic)
                    for (SizeType i = 0; i < m_iGraphSize; i++) {
                        ...
                        if ((i * 5) % m_iGraphSize == 0)
                            TLOG_INFO("Refine {} {}%%", iter,
                                         static_cast<int>(i * 1.0 / m_iGraphSize * 100));
                    }
                    auto t2 = std::chrono::high_resolution_clock::now();
                    TLOG_INFO("Refine RNG time (s): {} Graph Acc: {}",
                                 std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count(),
                                 GraphAccuracyEstimation(index, 100, idmap));
                }
...
#pragma omp parallel for schedule(dynamic)
                    for (SizeType i = 0; i < m_iGraphSize; i++) {
                        ...
                        if ((i * 5) % m_iGraphSize == 0)
                            TLOG_INFO("Refine {} {}%%", m_iRefineIter - 1,
                                         static_cast<int>(i * 1.0 / m_iGraphSize * 100));
                    }
                    auto t2 = std::chrono::high_resolution_clock::now();
                    TLOG_INFO("Refine RNG time (s): {} Graph Acc: {}",
            ...
            template<typename T>
            ErrorCode
            RefineGraph(VectorIndex *index, std::vector<SizeType> &indices, std::vector<SizeType> &reverseIndices,
                        std::shared_ptr<Helper::DiskIO> output, NeighborhoodGraph *newGraph,
                        const std::unordered_map<SizeType, SizeType> *idmap = nullptr) {
                ...
                newGraph->m_pNeighborhoodGraph.Initialize(R, m_iNeighborhoodSize, index->m_iDataBlockSize,
                                                          index->m_iDataCapacity);
...
#pragma omp parallel for schedule(dynamic)
                for (SizeType i = 0; i < R; i++) {
                    if ((i * 5) % R == 0)
                        TLOG_INFO("Refine {}%%", static_cast<int>(i * 1.0 / R * 100));
            ...
            template<typename T>
            void
            RefineNode(VectorIndex *index, const SizeType node, bool updateNeighbors, bool searchDeleted, int CEF) {
            ...
            ErrorCode SaveGraph(std::string sGraphFilename) const {
                TLOG_INFO("Save {} To {}", m_pNeighborhoodGraph.Name().c_str(),
                             sGraphFilename.c_str());
                auto ptr = f_createIO();
            ...
            ErrorCode SaveGraph(std::shared_ptr<Helper::DiskIO> output) const {
                ...
                for (int i = 0; i < m_iGraphSize; i++)
                    IOBINARY(output, WriteBinary, sizeof(SizeType) * m_iNeighborhoodSize,
                             (char *) m_pNeighborhoodGraph[i]);
                TLOG_INFO("Save {} ({},{}) Finish!",
...
#endif  // TANN_GRAPH_NEIGHBORHOOD_GRAPH_H_
