...
#ifndef TANN_GRAPH_RELATIVE_NEAREST_NEIGHBORHOOD_GRAPH_H_
#define TANN_GRAPH_RELATIVE_NEAREST_NEIGHBORHOOD_GRAPH_H_
    ...
    namespace COMMON {
        ...
        public:
            ...
            RebuildNeighbors(VectorIndex *index, const SizeType node, SizeType *nodes, const BasicResult *queryResults,
                ...
                for (int j = 0; j < numResults && count < m_iNeighborhoodSize; j++) {
                    ...
                    for (DimensionType k = 0; k < count; k++) {
                        if (m_fRNGFactor *
                            index->ComputeDistance(index->GetSample(nodes[k]), index->GetSample(item.VID)) <
                            item.Dist) {
                            good = false;
                            break;
                        }
            ...
            void InsertNeighbors(VectorIndex *index, const SizeType node, SizeType insertNode, float insertDist) {
                ...
                float tmpDist;
                const void *tmpVec;
                int checkNeighborhoodSize = (nodes[m_iNeighborhoodSize - 1] < -1) ? m_iNeighborhoodSize - 1
                                                                                  : m_iNeighborhoodSize;
                for (DimensionType k = 0; k < checkNeighborhoodSize; k++) {
                    ...
                    if (tmpDist > insertDist || (insertDist == tmpDist && insertNode < tmpNode)) {
                        nodes[k] = insertNode;
                        while (++k < checkNeighborhoodSize &&
                               index->ComputeDistance(tmpVec, nodeVec) <= index->ComputeDistance(tmpVec, insertVec)) {
...
#endif  // TANN_GRAPH_RELATIVE_NEAREST_NEIGHBORHOOD_GRAPH_H_
