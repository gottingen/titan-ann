...
{
    tann::prefetch_vector((const char *)_data + _aligned_dim * (size_t)loc, sizeof(data_t) * _aligned_dim);
}
...
template TURBO_DLL class InMemDataStore<float>;
template TURBO_DLL class InMemDataStore<int8_t>;
template TURBO_DLL class InMemDataStore<uint8_t>;
