...
inline void print_error_and_terminate(std::stringstream &error_stream)
{
    ...
    throw tann::ANNException(error_stream.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
}
...
{
    tann::cout << "Reading bin file " << bin_file.c_str() << " ..." << std::endl;
    std::ifstream reader;
    reader.exceptions(std::ifstream::failbit | std::ifstream::badbit);

    try
    {
        tann::cout << "Opening bin file " << bin_file.c_str() << "... " << std::endl;
                                ...
                                uint64_t &gt_num)
{
    size_t read_blk_size = 64 * 1024 * 1024;
    cached_ifstream reader(bin_file, read_blk_size);
    tann::cout << "Reading truthset file " << bin_file.c_str() << "... " << std::flush;
    size_t actual_file_size = reader.get_file_size();
    ...
    tann::cout << "Metadata: #pts = " << gt_num << ", #total_results = " << total_res << "..." << std::endl;
...
inline size_t save_bin(const std::string &filename, T *data, size_t npts, size_t ndims, size_t offset = 0)
{
    ...
    tann::cout << "bin: #pts = " << npts << ", #dims = " << ndims << ", size = " << bytes_written << "B"
                  << std::endl;
                                  ...
                                  size_t &dim, size_t &rounded_dim)
{
    ...
    tann::cout << "Metadata: #pts = " << npts << ", #dims = " << dim << ", aligned_dim = " << rounded_dim << "... "
                  << std::flush;
    size_t allocSize = npts * rounded_dim * sizeof(T);
    tann::cout << "allocating aligned memory of " << allocSize << " bytes... " << std::flush;
    alloc_aligned(((void **)&data), allocSize, 8 * sizeof(T));
    tann::cout << "done. Copying data to mem_aligned buffer..." << std::flush;
...
inline void load_aligned_bin(const std::string &bin_file, T *&data, size_t &npts, size_t &dim, size_t &rounded_dim)
{
    ...
    try
    {
        tann::cout << "Reading (with alignment) bin file " << bin_file << " ..." << std::flush;
        reader.open(bin_file, std::ios::binary | std::ios::ate);
                                        ...
                                        const size_t &rounded_dim, size_t offset = 0)
{
    if (data == nullptr)
    {
        tann::cerr << "Memory was not allocated for " << data << " before calling the load function. Exiting..."
                      << std::endl;
        throw tann::ANNException("Null pointer passed to copy_aligned_data_from_file function", -1, __FUNCSIG__,
                                    __FILE__, __LINE__);
    }
    std::ifstream reader;
...
{
    if (!in.is_open())
        throw tann::ANNException("Index file size check called on unopened file stream", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    in.seekg(0, in.end);
    ...
    if (actual_file_size != expected_file_size)
    {
        tann::cerr << "Index file size error. Expected size (metadata): " << expected_file_size
...
inline std::vector<std::string> read_file_to_vector_of_strings(const std::string &filename, bool unique = false)
{
    ...
    {
        throw tann::ANNException(std::string("Failed to open file. filename can not be blank"), -1);
    }
    return result;
}
        ...
        for (const auto &path : paths_to_clean)
        {
            for (const auto &suffix : path_suffixes)
            {
                ...
                if (std::remove(curr_path_to_clean.c_str()) != 0)
                    tann::cout << "Warning: Unable to remove file :" << curr_path_to_clean << std::endl;
            }
        }
        tann::cout << "Cleaned all artifacts" << std::endl;
    }
    catch (const std::exception &e)
    {
        tann::cout << "Warning: Unable to clean all artifacts " << e.what() << std::endl;
    }
}
