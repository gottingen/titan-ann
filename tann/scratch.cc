...
template <typename T> SSDQueryScratch<T>::SSDQueryScratch(size_t aligned_dim, size_t visited_reserve)
{
    ...
    tann::alloc_aligned((void **)&sector_scratch, (size_t)MAX_N_SECTOR_READS * (size_t)SECTOR_LEN, SECTOR_LEN);
    tann::alloc_aligned((void **)&aligned_query_T, aligned_dim * sizeof(T), 8 * sizeof(T));
...
template TURBO_DLL class SSDQueryScratch<int8_t>;
template TURBO_DLL class SSDQueryScratch<uint8_t>;
template TURBO_DLL class SSDQueryScratch<float>;

template TURBO_DLL class SSDThreadData<int8_t>;
template TURBO_DLL class SSDThreadData<uint8_t>;
