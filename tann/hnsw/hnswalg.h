...
//                    if (candidate_id == 0) continue;
        ...
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
        searchBaseLayerST(tableint ep_id, const void *data_point, HnswSelector* selector, size_t ef) const {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
            ...
            if ((!has_deletions || !isMarkedDeleted(ep_id))
                && selector->is_valid(getExternalLabel(ep_id))) {
              dist_t dist = fstdistfunc_(data_point, getDataByInternalId(ep_id), dist_func_param_);
              lowerBound = dist;
              top_candidates.emplace(dist, ep_id);
...
//                    if (candidate_id == 0) continue;
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag)) {
                        ...
                        if (top_candidates.size() < ef || lowerBound > dist) {
                            ...
                            if ((!has_deletions || !isMarkedDeleted(candidate_id))
                                && selector->is_valid(uint32_t(getExternalLabel(candidate_id))))
        ...
        searchKnn(const void *query_data, HnswSelector* selector, size_t k) const {
            ...
            if (num_deleted_) {
                top_candidates=searchBaseLayerST<true,true>(
                        currObj, query_data, selector, std::max(ef_, k));
            }
            else{
