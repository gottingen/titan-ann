...
#ifndef TANN_QUANTIZER_PQ_QUANTIZER_H_
#define TANN_QUANTIZER_PQ_QUANTIZER_H_
...
namespace tann
{
    namespace COMMON
    {
        ...
        float PQQuantizer<T>::CosineDistance(const std::uint8_t* pX, const std::uint8_t* pY) const
            // pX must be query distance table for ADC
        {
            TLOG_ERROR("Quantizer does not support CosineDistance!");
            return 0;
        }
        ...
        ErrorCode PQQuantizer<T>::SaveQuantizer(std::shared_ptr<Helper::DiskIO> p_out) const
        {
            ...
            TLOG_INFO("Saving quantizer: Subvectors:{} KsPerSubvector:{} DimPerSubvector:{}", m_NumSubvectors, m_KsPerSubvector, m_DimPerSubvector);
...
#endif // TANN_QUANTIZER_PQ_QUANTIZER_H_
