...
template<typename T>
void getCandidates(tann::VectorIndex* index, int numVectors, int candidatesPerVector, int* candidates) {

#pragma omp parallel for schedule(dynamic)
  for (SizeType i = 0; i < numVectors; i++)
  {
    tann::COMMON::QueryResultSet<T> query((const T*)index->GetSample(i), candidatesPerVector);
     index->SearchTree(query);
     for (tann::DimensionType j = 0; j < candidatesPerVector; j++) {
...
void refineGraphGPU(tann::VectorIndex* index, Point<T,SUMTYPE,MAX_DIM>* d_points, int* d_graph, int dataSize, int KVAL, int candidatesPerVector, int refineDepth, int refines, int metric) {
  ...
  SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Info, "find candidates time (ms): %lld\n", std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count());
...
// Kernel that refines a batch of points' KNN neighbors into RNG neighbors
      ...
      if(status != cudaSuccess) {
          SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Error, "Refine error code:%d\n", status);
      }
    }

  }
  t2 = std::chrono::high_resolution_clock::now();
  SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Info, "GPU refine time (ms): %lld\n", std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count());
