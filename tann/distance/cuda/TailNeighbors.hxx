...
template<typename T>
void extractAndCopyHeadRaw_multi(T* dataBuffer, tann::VectorIndex* headIndex, T** d_headRaw, size_t headRows, int dim, int NUM_GPUS) {
  T* vecPtr;
...
template<typename T, typename SUMTYPE>
void getTailNeighborsTPT(T* vectors, tann::SizeType N, tann::VectorIndex* headIndex, std::unordered_set<int>& headVectorIDS, int dim, int RNG_SIZE, int numThreads, int NUM_TREES, int LEAF_SIZE, int metric, int NUM_GPUS, Edge* selections) {
    ...
    if(numDevicesOnHost < NUM_GPUS) {
      SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Error, "NumGPUs parameter %d, but only %d devices available on system.  Exiting.\n", NUM_GPUS, numDevicesOnHost);
      exit(1);
    }

    SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Info, "Building SSD index with %d GPUs...\n", NUM_GPUS);
...
/***** Device variables *****/
    ...
    SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Info, "Setting up each of the %d GPUs...\n", NUM_GPUS);
...
/*** Compute batch sizes and allocate all data on GPU ***/
    for(int gpuNum=0; gpuNum < NUM_GPUS; gpuNum++) {
        ...
        SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "GPU test/warmup complete - kernel status:%d\n", resultErr);
 ...
 *****************************************************/
        if(BATCH_SIZE[gpuNum] == 0 || ((int)pointsPerGPU[gpuNum]) / BATCH_SIZE[gpuNum] > 10000) {
            SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Error, "Insufficient GPU memory to build SSD index on GPU %d.  Available GPU memory:%lu MB, Head index requires:%lu MB, leaving a maximum batch size of %d elements, which is too small to run efficiently.\n", gpuNum, (freeMem)/1000000, (headVecSize+treeSize)/1000000, maxEltsPerBatch);
            exit(1);
        }
        SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Info, "Memory for head vectors:%lu MiB, Memory for TP trees:%lu MiB, Memory left for tail vectors:%lu MiB, total tail vectors:%lu, batch size:%d, total batches:%d\n", headVecSize/1000000, treeSize/1000000, tailMemAvail/1000000, pointsPerGPU[gpuNum], BATCH_SIZE[gpuNum], (((BATCH_SIZE[gpuNum]-1)+pointsPerGPU[gpuNum]) / BATCH_SIZE[gpuNum]));
        SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "Allocating GPU memory: tail points:%lu MiB, head points:%lu MiB, results:%lu MiB, TPT:%lu MiB, Total:%lu MiB\n", (BATCH_SIZE[gpuNum]*sizeof(T))/1000000, (headRows*sizeof(T))/1000000, (BATCH_SIZE[gpuNum]*RNG_SIZE*sizeof(DistPair<SUMTYPE>))/1000000, (sizeof(TPtree))/1000000, ((BATCH_SIZE[gpuNum]+headRows)*sizeof(T) + (BATCH_SIZE[gpuNum]*RNG_SIZE*sizeof(DistPair<SUMTYPE>)) + (sizeof(TPtree)))/1000000);
        ...
        SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "tpt structure initialized for %lu head vectors, %d levels, leaf size:%d\n", headRows, TPTlevels, LEAF_SIZE);
    ...
    while(!done) { // Continue until all GPUs have completed all of their batches 
        ...
        for(int gpuNum=0; gpuNum<NUM_GPUS; ++gpuNum) {
            ...
            SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Info, "GPU %d - starting batch with GPUOffset:%lu, offset:%lu, total offset:%lu, size:%lu, TailRows:%lld\n", gpuNum, GPUPointOffset[gpuNum], offset[gpuNum], GPUPointOffset[gpuNum]+offset[gpuNum], curr_batch_size[gpuNum], pointsPerGPU[gpuNum]);
            ...
            SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "Copied %lu tail points to GPU - kernel status:%d\n", curr_batch_size[gpuNum], resultErr);
           ...
           for(int i=0; i<curr_batch_size[gpuNum]*RNG_SIZE; i+=COPY_BUFF_SIZE*RNG_SIZE) {
            ...
            SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "Copying initialized result list to GPU - batch size:%lu - replica count:%d, copy bytes:%lu, GPU offset:%lu, total result offset:%lu, - kernel status:%d\n", curr_batch_size[gpuNum], RNG_SIZE, curr_batch_size[gpuNum]*RNG_SIZE*sizeof(DistPair<SUMTYPE>), GPUPointOffset[gpuNum], (GPUPointOffset[gpuNum]+offset[gpuNum])*RNG_SIZE, resultErr);
        }
...
auto t1 = std::chrono::high_resolution_clock::now();
            ...
            CUDA_CHECK(cudaDeviceSynchronize());
            SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "TPT %d created on all GPUs\n", tree_id);
...
auto t2 = std::chrono::high_resolution_clock::now();
            ...
            CUDA_CHECK(cudaDeviceSynchronize());
            SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "All GPUs finished finding neighbors of tails using TPT %d\n", tree_id);

auto t3 = std::chrono::high_resolution_clock::now();

SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "Tree %d complete, time to build tree:%.2lf, time to compute tail neighbors:%.2lf\n", tree_id, GET_CHRONO_TIME(t1, t2), GET_CHRONO_TIME(t2, t3));

        } // TPT loop

        SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "Batch complete on all GPUs, copying results back to CPU...\n");


        // Copy results of batch from each GPU to CPU result set
        for(int gpuNum=0; gpuNum < NUM_GPUS; ++gpuNum) {
            CUDA_CHECK(cudaSetDevice(gpuNum));
SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "gpu:%d, copying results size %lu to results offset:%d*%d=%d\n", gpuNum, curr_batch_size[gpuNum], GPUPointOffset[gpuNum]+offset[gpuNum],RNG_SIZE, (GPUPointOffset[gpuNum]+offset[gpuNum])*RNG_SIZE);
        ...
        SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "Finished copying all batch results back to CPU\n");
    ...
    SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "GPU SSD build complete.  Freeing GPU memory...\n");
...
// Maximum number of elements that can be sorted on GPU
  ...
  SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Info, "Sorting final results. Size of result:%ld elements = %0.2lf GB, Available GPU memory:%0.2lf, sorting in %d batches\n", N, ((double)(N*sizeof(GPUEdge))/1000000000.0), ((double)freeMem)/1000000000.0, num_batches);
  ...
  SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "Allocating %ld bytes on GPU for sorting\n", sortBatchSize*sizeof(GPUEdge));
  ...
  for(size_t startIdx = 0; startIdx < N; startIdx += sortBatchSize) {
    ...
    SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Debug, "Sorting batch id:%ld, size:%ld\n", startIdx, batchSize);
    ...
    try {
      ...
      SPTAGLIB_LOG(tann::Helper::LogLevel::LL_Info, "Error: %s \n",e.what());
    }
...
void extractAndCopyHeadPoints(Point<T,SUMTYPE,MAX_DIM>* headPointBuffer, T* vectors, tann::VectorIndex* headIndex, Point<T,SUMTYPE,MAX_DIM>* d_headPoints, size_t headRows, int dim) {
...
template<typename T, typename SUMTYPE, int MAX_DIM>
void getTailNeighborsGPU(T* vectors, tann::SizeType N, std::shared_ptr<tann::VectorIndex>& headIndex, std::unordered_set<int> headVectorIDS, int dim, DistPair<SUMTYPE>* results, int resultsPerVector, int BATCH_SIZE, int searchDepth, int numThreads, int metric) {
...
// Get candidates from search on CPU
        ...
        for(size_t fullID=0; fullID < curr_batch_size; fullID++) {
            ...
            tann::COMMON::QueryResultSet<T> query((T*)&tailPoints[offset+fullID].coords[0], SEEDS);
