...
namespace tann {
    ...
    void generate_label_indices(path input_data_path, path final_index_path_prefix, label_set all_labels, uint32_t R,
        ...
        tann::IndexWriteParameters label_index_build_parameters = tann::IndexWriteParametersBuilder(L, R)
        ...
        for (const auto &lbl: all_labels) {
            ...
            tann::get_bin_metadata(curr_label_input_data_path, number_of_label_points, dimension);
 ...
 */
    ...
    template TURBO_DLL void generate_label_indices<float>(path input_data_path, path final_index_path_prefix,
                                                          label_set all_labels, uint32_t R, uint32_t L, float alpha,
                                                          uint32_t num_threads);

    template TURBO_DLL void generate_label_indices<uint8_t>(path input_data_path, path final_index_path_prefix,
                                                            label_set all_labels, uint32_t R, uint32_t L,
                                                            float alpha, uint32_t num_threads);

    template TURBO_DLL void generate_label_indices<int8_t>(path input_data_path, path final_index_path_prefix,
                                                           label_set all_labels, uint32_t R, uint32_t L,
                                                           float alpha, uint32_t num_threads);
