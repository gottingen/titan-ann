...
namespace tann {
    ...
    void add_new_file_to_single_index(std::string index_file, std::string new_file) {
        ...
        uint64_t nr, nc;
        tann::load_bin<uint64_t>(index_file, metadata, nr, nc);
        if (nc != 1) {
        ...
        tann::save_bin<uint64_t>(index_file, new_meta.data(), new_meta.size(), 1);
    }
    ...
    size_t calculate_num_pq_chunks(double final_index_ram_limit, size_t points_num, uint32_t dim,
        ...
        tann::cout << "Calculated num_pq_chunks :" << num_pq_chunks << std::endl;
        if (param_list.size() >= 6) {
            ...
            if (compress_ratio > 0 && compress_ratio <= 1) {
                ...
                if (chunks_by_cr > 0 && chunks_by_cr < num_pq_chunks) {
                    tann::cout << "Compress ratio:" << compress_ratio << " new #pq_chunks:" << chunks_by_cr
                               << std::endl;
                ...
                tann::cerr << "Compression ratio: " << compress_ratio << " should be in (0,1]" << std::endl;
            }
        }
        ...
        tann::cout << "Compressing " << dim << "-dimensional data into " << num_pq_chunks << " bytes per vector."
                   << std::endl;
    ...
    T *generateRandomWarmup(uint64_t warmup_num, uint64_t warmup_dim, uint64_t warmup_aligned_dim) {
        T *warmup = nullptr;
        warmup_num = 100000;
        tann::cout << "Generating random warmup file with dim " << warmup_dim << " and aligned dim "
 ...
 ***************************************************/
    ...
    merge_shards(const std::string &vamana_prefix, const std::string &vamana_suffix, const std::string &idmaps_prefix,
        ...
        nnodes++;
        tann::cout << "# nodes: " << nnodes << ", max. degree: " << max_degree << std::endl;

        // compute inverse map: node -> shards
        std::vector<std::pair<uint32_t, uint32_t>> node_shard;
        node_shard.reserve(nelems);
        for (size_t shard = 0; shard < nshards; shard++) {
            tann::cout << "Creating inverse map -- shard #" << shard << std::endl;
        ...
        tann::cout << "Finished computing node -> shards map" << std::endl;
        ...
        tann::cout << "Max input width: " << max_input_width << ", output width: " << output_width << std::endl;
        ...
        tann::cout << "Starting merge" << std::endl;
        ...
        for (const auto &id_shard: node_shard) {
            ...
            if (shard_nnbrs == 0) {
                tann::cout << "WARNING: shard #" << shard_id << ", node_id " << node_id << " has 0 nbrs" << std::endl;
            }
...
/* If the number of filters per point N exceeds the graph degree R,
  ...
  new nodes added to the graph */
    ...
    void breakup_dense_points(const std::string data_file, const std::string labels_file, uint32_t density,
        ...
        T *data;
        uint64_t npts, ndims;
        tann::load_bin<T>(data_file, data, npts, ndims);
        ...
        tann::cout << "fraction of dense points with >= " << density << " labels = " << (float) dense_pts / (float) npts
                   << std::endl;

        if (labels_per_point.size() != 0) {
            tann::cout << labels_per_point.size() << " is the new number of points" << std::endl;
        ...
        if (dummy_pt_ids.size() != 0) {
            tann::cout << dummy_pt_ids.size() << " is the number of dummy points created" << std::endl;
        ...
        tann::save_bin<T>(out_data_file, data, labels_per_point.size(), ndims);
    }
    ...
    void extract_shard_labels(const std::string &in_label_file, const std::string &shard_ids_bin,
        ...
        tann::cout << "Extracting labels for shard" << std::endl;

        uint32_t *ids = nullptr;
        uint64_t num_ids, tmp_dim;
        tann::load_bin(shard_ids_bin, ids, num_ids, tmp_dim);
    ...
    int build_merged_vamana_index(std::string base_file, tann::Metric compareMetric, uint32_t L, uint32_t R,
        ...
        tann::get_bin_metadata(base_file, base_num, base_dim);
        ...
        if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
            tann::cout << "Full index fits in RAM budget, should consume at most "
            ...
            tann::IndexWriteParameters paras = tann::IndexWriteParametersBuilder(L, R)
                    ...
                    .build();
            using TagT = uint32_t;
            tann::Index<T, TagT, LabelT> _index(compareMetric, base_dim, base_num, false, false, false,
        ...
        tann::cout << timer.elapsed_seconds_for_step("partitioning data") << std::endl;
        ...
        for (int p = 0; p < num_parts; p++) {
            ...
            tann::IndexWriteParameters paras =
                    tann::IndexWriteParametersBuilder(L, (2 * R / 3)).with_filter_list_size(Lf).build();

            uint64_t shard_base_dim, shard_base_pts;
            get_bin_metadata(shard_base_file, shard_base_pts, shard_base_dim);
            tann::Index<T> _index(compareMetric, shard_base_dim, shard_base_pts, false, false, false,
                                  build_pq_bytes > 0,
                                  build_pq_bytes, use_opq);
            if (!use_filters) {
                _index.build(shard_base_file.c_str(), shard_base_pts, paras);
            } else {
                tann::extract_shard_labels(label_file, shard_ids_file, shard_labels_file);
                if (universal_label != "") { //  indicates no universal label
                    LabelT unv_label_as_num = 0;
                    _index.set_universal_label(unv_label_as_num);
                }
        ...
        tann::cout << timer.elapsed_seconds_for_step("building indices on shards") << std::endl;

        timer.reset();
        tann::merge_shards(merged_index_prefix + "_subshard-", "_mem.index", merged_index_prefix + "_subshard-",
                           "_ids_uint32.bin", num_parts, R, mem_index_path, medoids_file, use_filters,
                           labels_to_medoids_file);
        tann::cout << timer.elapsed_seconds_for_step("merging indices") << std::endl;
...
// 99.9 latency not blowing up
    template<typename T, typename LabelT>
    uint32_t optimize_beamwidth(std::unique_ptr<tann::PQFlashIndex<T, LabelT>> &pFlashIndex, T *tuning_sample,
        ...
        while (!stop_flag) {
            ...
            tann::QueryStats *stats = new tann::QueryStats[tuning_sample_num];
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
            ...
            double lat_999 = tann::get_percentile_stats<float>(
                    stats, tuning_sample_num, 0.999f, [](const tann::QueryStats &stats) { return stats.total_us; });

            double mean_latency = tann::get_mean_stats<float>(
                    stats, tuning_sample_num, [](const tann::QueryStats &stats) { return stats.total_us; });
    ...
    create_disk_layout(const std::string base_file, const std::string mem_index_file, const std::string output_file,
        ...
        tann::cout << "medoid: " << medoid << "B" << std::endl;
        tann::cout << "max_node_len: " << max_node_len << "B" << std::endl;
        tann::cout << "nnodes_per_sector: " << nnodes_per_sector << "B" << std::endl;
        ...
        tann::cout << "# sectors: " << n_sectors << std::endl;
        uint64_t cur_node_id = 0;
        for (uint64_t sector = 0; sector < n_sectors; sector++) {
            if (sector % 100000 == 0) {
                tann::cout << "Sector #" << sector << "written" << std::endl;
            }
            memset(sector_buf.get(), 0, SECTOR_LEN);
            for (uint64_t sector_node_id = 0;
                 sector_node_id < nnodes_per_sector && cur_node_id < npts_64; sector_node_id++) {
        ...
        if (append_reorder_data) {
            tann::cout << "Index written. Appending reorder data..." << std::endl;
            ...
            for (uint64_t sector = 0; sector < n_reorder_sectors; sector++) {
                if (sector % 100000 == 0) {
                    tann::cout << "Reorder data Sector #" << sector << "written" << std::endl;
                }
        ...
        tann_writer.close();
        tann::save_bin<uint64_t>(output_file, output_file_meta.data(), output_file_meta.size(), 1, 0);
        tann::cout << "Output disk index file written to " << output_file << std::endl;
    }

    template<typename T, typename LabelT>
    int build_disk_index(const char *dataFilePath, const char *indexFilePath, const char *indexBuildParameters,
                         tann::Metric compareMetric, bool use_opq, const std::string &codebook_prefix, bool use_filters,
        ...
        if (param_list.size() < 5 || param_list.size() > 9) {
            tann::cout << "Correct usage of parameters is R (max degree)\n"
        ...
        if (!std::is_same<T, float>::value && compareMetric == tann::Metric::INNER_PRODUCT) {
            std::stringstream stream;
        ...
        if (compareMetric == tann::Metric::INNER_PRODUCT) {
            ...
            float max_norm_of_base = tann::prepare_base_for_inner_products<T>(base_file, prepped_base);
            std::string norm_file = disk_index_path + "_max_base_norm.bin";
            tann::save_bin<float>(norm_file, &max_norm_of_base, 1, 1);
            tann::cout << timer.elapsed_seconds_for_step("preprocessing data for inner product") << std::endl;
        }
        ...
        tann::cout << "Starting index build: R=" << R << " L=" << L << " Query RAM budget: " << final_index_ram_limit
        ...
        Timer timer;
        tann::get_bin_metadata(data_file_to_use.c_str(), points_num, dim);
        ...
        tann::cout << timer.elapsed_seconds_for_step("generating quantized data") << std::endl;
...
// This code should only be available in the "build" DLL.
        ...
        timer.reset();
        tann::build_merged_vamana_index<T, LabelT>(data_file_to_use.c_str(), tann::Metric::L2, L, R, p_val,
                                                   indexing_ram_budget, mem_index_path, medoids_path, centroids_path,
                                                   build_pq_bytes, use_opq, num_threads, use_filters,
                                                   labels_file_to_use,
                                                   labels_to_medoids_path, universal_label, Lf);
        tann::cout << timer.elapsed_seconds_for_step("building merged vamana index") << std::endl;

        timer.reset();
        if (!use_disk_pq) {
            tann::create_disk_layout<T>(data_file_to_use.c_str(), mem_index_path, disk_index_path);
        } else {
            if (!reorder_data)
                tann::create_disk_layout<uint8_t>(disk_pq_compressed_vectors_path, mem_index_path, disk_index_path);
            else
                tann::create_disk_layout<uint8_t>(disk_pq_compressed_vectors_path, mem_index_path, disk_index_path,
                                                  data_file_to_use.c_str());
        }
        tann::cout << timer.elapsed_seconds_for_step("generating disk layout") << std::endl;
        ...
        auto e = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> diff = e - s;
        tann::cout << "Indexing time: " << diff.count() << std::endl;

        return 0;
    }

    template TURBO_DLL void create_disk_layout<int8_t>(const std::string base_file,
                                                       const std::string mem_index_file,
                                                       const std::string output_file,
                                                       const std::string reorder_data_file);

    template TURBO_DLL void create_disk_layout<uint8_t>(const std::string base_file,
                                                        const std::string mem_index_file,
                                                        const std::string output_file,
                                                        const std::string reorder_data_file);

    template TURBO_DLL void create_disk_layout<float>(const std::string base_file, const std::string mem_index_file,
                                                      const std::string output_file,
                                                      const std::string reorder_data_file);

    template TURBO_DLL int8_t *load_warmup<int8_t>(const std::string &cache_warmup_file, uint64_t &warmup_num,
                                                   uint64_t warmup_dim, uint64_t warmup_aligned_dim);

    template TURBO_DLL uint8_t *load_warmup<uint8_t>(const std::string &cache_warmup_file, uint64_t &warmup_num,
                                                     uint64_t warmup_dim, uint64_t warmup_aligned_dim);
    ...
    template TURBO_DLL uint32_t optimize_beamwidth<uint8_t, uint32_t>(
            std::unique_ptr<tann::PQFlashIndex<uint8_t, uint32_t>> &pFlashIndex, uint8_t *tuning_sample,
            uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads,
            uint32_t start_bw);

    template TURBO_DLL uint32_t optimize_beamwidth<float, uint32_t>(
            std::unique_ptr<tann::PQFlashIndex<float, uint32_t>> &pFlashIndex, float *tuning_sample,
            uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads,
            uint32_t start_bw);

    template TURBO_DLL uint32_t optimize_beamwidth<int8_t, uint16_t>(
            std::unique_ptr<tann::PQFlashIndex<int8_t, uint16_t>> &pFlashIndex, int8_t *tuning_sample,
            uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads,
            uint32_t start_bw);

    template TURBO_DLL uint32_t optimize_beamwidth<uint8_t, uint16_t>(
            std::unique_ptr<tann::PQFlashIndex<uint8_t, uint16_t>> &pFlashIndex, uint8_t *tuning_sample,
            uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads,
            uint32_t start_bw);

    template TURBO_DLL uint32_t optimize_beamwidth<float, uint16_t>(
            std::unique_ptr<tann::PQFlashIndex<float, uint16_t>> &pFlashIndex, float *tuning_sample,
            uint64_t tuning_sample_num, uint64_t tuning_sample_aligned_dim, uint32_t L, uint32_t nthreads,
            uint32_t start_bw);

    template TURBO_DLL int build_disk_index<int8_t, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                              const char *indexBuildParameters,
                                                              tann::Metric compareMetric, bool use_opq,
                                                              const std::string &codebook_prefix, bool use_filters,
                                                              const std::string &label_file,
                                                              const std::string &universal_label,
                                                              const uint32_t filter_threshold, const uint32_t Lf);

    template TURBO_DLL int build_disk_index<uint8_t, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                               const char *indexBuildParameters,
                                                               tann::Metric compareMetric, bool use_opq,
                                                               const std::string &codebook_prefix, bool use_filters,
                                                               const std::string &label_file,
    ...
    template TURBO_DLL int build_disk_index<float, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                             const char *indexBuildParameters,
                                                             tann::Metric compareMetric, bool use_opq,
                                                             const std::string &codebook_prefix, bool use_filters,
                                                             const std::string &label_file,
...
// LabelT = uint16
    template TURBO_DLL int build_disk_index<int8_t, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                              const char *indexBuildParameters,
                                                              tann::Metric compareMetric, bool use_opq,
                                                              const std::string &codebook_prefix, bool use_filters,
                                                              const std::string &label_file,
    ...
    template TURBO_DLL int build_disk_index<uint8_t, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                               const char *indexBuildParameters,
                                                               tann::Metric compareMetric, bool use_opq,
                                                               const std::string &codebook_prefix, bool use_filters,
                                                               const std::string &label_file,
    ...
    template TURBO_DLL int build_disk_index<float, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                             const char *indexBuildParameters,
                                                             tann::Metric compareMetric, bool use_opq,
                                                             const std::string &codebook_prefix, bool use_filters,
                                                             const std::string &label_file,
    ...
    template TURBO_DLL int build_merged_vamana_index<int8_t, uint32_t>(
            std::string base_file, tann::Metric compareMetric, uint32_t L, uint32_t R, double sampling_rate,
