...
namespace tann {
    ...
    PQFlashIndex<T, LabelT>::PQFlashIndex(std::shared_ptr<AlignedFileReader> &fileReader, tann::Metric m)
        ...
        this->dist_cmp.reset(tann::get_distance_function<T>(metric));
        this->dist_cmp_float.reset(tann::get_distance_function<float>(metric));
    }
    ...
    PQFlashIndex<T, LabelT>::~PQFlashIndex() {
        ...
        if (nhood_cache_buf != nullptr) {
            ...
            tann::aligned_free(coord_cache_buf);
        }
    ...
    void PQFlashIndex<T, LabelT>::setup_thread_data(uint64_t nthreads, uint64_t visited_reserve) {
        tann::cout << "Setting up thread-specific contexts for nthreads: " << nthreads << std::endl;
...
#pragma omp critical
    ...
    void PQFlashIndex<T, LabelT>::load_cache_list(std::vector<uint32_t> &node_list) {
        ...
        tann::alloc_aligned((void **) &coord_cache_buf, coord_cache_buf_len * sizeof(T), 8 * sizeof(T));
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
    ...
    void PQFlashIndex<T, LabelT>::cache_bfs_levels(uint64_t num_nodes_to_cache, std::vector<uint32_t> &node_list,
        ...
        if (num_nodes_to_cache > tenp_nodes) {
            tann::cout << "Reducing nodes to cache from: " << num_nodes_to_cache << " to: " << tenp_nodes
...
continue;
        ...
        tann::cout << "Level: " << lvl << std::flush;
        tann::cout << ". #nodes: " << node_list.size() - prev_node_set_size << ", #nodes thus far: " << node_list.size()
                   << std::endl;
        tann::cout << "done" << std::endl;
    }
    ...
    void PQFlashIndex<T, LabelT>::use_medoids_data_as_centroids() {
        ...
        tann::cout << "Loading centroid data from medoids vector data of " << num_medoids << " medoid(s)" << std::endl;
    ...
    void PQFlashIndex<T, LabelT>::get_label_file_metadata(std::string map_file, uint32_t &num_pts,
        ...
        tann::cout << "Labels file metadata: num_points: " << num_pts << ", #total_labels: " << num_total_labels
                   << std::endl;
        infile.close();
    }
    ...
    void PQFlashIndex<T, LabelT>::set_universal_label(const LabelT &label) {
        ...
        if (temp_filter_num == -1) {
            tann::cout << "Error, could not find universal label." << std::endl;
        } else {
    ...
    int PQFlashIndex<T, LabelT>::load_from_separate_paths(uint32_t num_threads, const char *index_filepath,
        ...
        if (pq_file_num_centroids != 256) {
            tann::cout << "Error. Number of PQ centroids is not 256. Exiting." << std::endl;
            return -1;
        }
        ...
        if (turbo::filesystem::exists(disk_pq_pivots_path)) {
            ...
            tann::cout << "Disk index uses PQ data compressed down to " << disk_pq_n_chunks << " bytes per point."
                       << std::endl;
        }
...
// read index metadata
        ...
        if (this->num_frozen_points == 1) {
            tann::cout << " Detected frozen point in index at location " << this->frozen_location
        ...
        if (turbo::filesystem::exists(norm_file) && metric == tann::Metric::INNER_PRODUCT) {
            ...
            tann::cout << "Setting re-scaling factor of base vectors to " << this->max_base_norm << std::endl;
            delete[] norm_val;
        }
        tann::cout << "done.." << std::endl;
        return 0;
    ...
    void PQFlashIndex<T, LabelT>::cached_beam_search(const T *query1, const uint64_t k_search, const uint64_t l_search,
        ...
        auto compute_dists = [this, pq_coord_scratch, pq_dists](const uint32_t *ids, const uint64_t n_ids,
                                                                float *dists_out) {
            tann::aggregate_coords(ids, n_ids, this->data, this->n_chunks, pq_coord_scratch);
            tann::pq_dist_lookup(pq_coord_scratch, n_ids, this->n_chunks, pq_dists, dists_out);
        };
        ...
        if (!use_filter) {
            for (uint64_t cur_m = 0; cur_m < num_medoids; cur_m++) {
                float cur_expanded_dist =
                        dist_cmp_float->compare(query_float, centroid_data + aligned_dim * cur_m,
        ...
        while (retset.has_unexpanded_node() && num_ios < io_limit) {
            ...
            if (!frontier.empty()) {
                ...
                for (uint64_t i = 0; i < frontier.size(); i++) {
                    ...
                    frontier_read_reqs.emplace_back(NODE_SECTOR_NO(((size_t) id)) * SECTOR_LEN, SECTOR_LEN,
                                                    fnhood.second);
                    if (stats != nullptr) {
                        stats->n_4k++;
                        stats->n_ios++;
...
// return value is the number of matching hits.
    ...
    template<typename T, typename LabelT>
    tann::Metric PQFlashIndex<T, LabelT>::get_metric() {
        return this->metric;
    }
