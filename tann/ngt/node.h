...
namespace tann {
    ...
    public:
        ...
        void setPivot(PersistentObject &f, VectorSpace &os, SharedMemoryAllocator &allocator) {
          if (pivot == 0) {
        pivot = tann::PersistentObject::allocate(os);
          }
          getPivot(os).set(f, os);
        }
        PersistentObject &getPivot(VectorSpace &os) {
          return *(PersistentObject*)os.getRepository().getAllocator().getAddr(pivot);
        }
        void deletePivot(VectorSpace &os, SharedMemoryAllocator &allocator) {
          os.deleteObject(&getPivot(os));
        }
#else // NGT_SHARED_MEMORY_ALLOCATOR

        void setPivot(tann::Object &f, VectorSpace &os) {
            if (pivot == 0) {
        ...
        void deletePivot(VectorSpace &os) {
    ...
    public:
        ...
        InternalNode(SharedMemoryAllocator &allocator, tann::VectorSpace *os = 0) : childrenSize(5) { initialize(allocator); }
#else

        InternalNode(size_t csize) : childrenSize(csize) { initialize(); }

        InternalNode(tann::VectorSpace *os = 0) : childrenSize(5) { initialize(); }
...
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
        void serialize(std::ofstream &os, SharedMemoryAllocator &allocator, VectorSpace *objectspace = 0) {
#else
...
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
        void serializeAsText(std::ofstream &os, SharedMemoryAllocator &allocator, VectorSpace *objectspace = 0) {
#else
...
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
        void deserializeAsText(std::ifstream &is, SharedMemoryAllocator &allocator, VectorSpace *objectspace = 0) {
#else
    ...
    public:
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
        LeafNode(SharedMemoryAllocator &allocator, tann::VectorSpace *os = 0) {
#else

        LeafNode(tann::VectorSpace *os = 0) {
#endif
            id = 0;
            id.setType(ID::Leaf);
            pivot = 0;
...
#else
            objectIDs = new tann::VectorDistance[LeafObjectsSizeMax];
#endif
#endif
        }
...
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
#ifndef NGT_NODE_USE_VECTOR
        tann::VectorDistance *getObjectIDs(SharedMemoryAllocator &allocator) {
          return (tann::VectorDistance *)allocator.getAddr(objectIDs);
        }
#endif
#else // NGT_SHARED_MEMORY_ALLOCATOR

        tann::VectorDistance *getObjectIDs() { return objectIDs; }
