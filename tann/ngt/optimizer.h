...
namespace tann {
    ...
    public:
        ...
        static void search(tann::Index &index, std::istream &gtStream, Command::SearchParameters &sp,
        ...
        static void
        search(tann::Index &index, std::istream &queries, std::istream &gtStream, Command::SearchParameters &sp,
               std::vector<MeasuredValue> &acc) {
        ...
        evaluate(std::istream &gtStream, std::istream &resultStream, SumupValues &sumupValues, std::string &type,
            ...
            if (recall) {
                if (specifiedResultSize == 0) {
                    TANN_THROW(turbo::Format("For calculating recalls, the result size should be specified."));
                }
                resultDataSize = specifiedResultSize;
            } else {
                checkAndGetSize(resultStream, resultDataSize);
            ...
            while (getline(gtStream, line)) {
                ...
                if (tokens[0] == "# Query No.") {
                    size_t n;
                    if (tokens.size() > 1 && turbo::SimpleAtoi(tokens[1], &n) && n == queryNo) {
                        std::unordered_set<size_t> gt;
        ...
        loadGroundTruth(std::istream &gtf, std::unordered_set<size_t> &gt, size_t resultDataSize, double &distance) {
            ...
            while (getline(gtf, line)) {
                if (line.size() != 0 && line.at(0) == '#') {
                    std::vector<std::string> gtf = turbo::StrSplit(line, "=");
                    if (gtf.size() >= 1) {
                        ...
                        if (gtf[0] == "# End of Query") {
                            if (searchCount != 1) {
                                TANN_THROW("Error: gt has not just one search result.");
                            }
                            if (dataCount < resultDataSize) {
                                TANN_THROW(turbo::Format("Error: gt data is less than result size! {}:{}", dataCount,
                                                         resultDataSize));
                            }
                            return;
                        }
                ...
                std::vector<std::string> result = turbo::StrSplit(line, " \t");
                if (result.size() < 3) {
                    TANN_THROW("result format is wrong. ");
                }
                size_t id;
                turbo::SimpleAtoi(result[1], &id);
                turbo::SimpleAtod(result[2], &distance);
                try {
                    gt.insert(id);
                } catch (...) {
                    TANN_THROW(turbo::Format("error {}", id));
                }
            }
        }
        ...
        static void checkAndGetSize(std::istream &resultStream, size_t &resultDataSize) {
            ...
            while (getline(resultStream, line)) {
                lineCount++;
                if (line.size() != 0 && line.at(0) == '#') {
                    std::vector<std::string> tf = turbo::StrSplit(line, "=");
                    if (tf.size() >= 1 && tf[0] == "# Query No.") {
                        ...
                        while (getline(resultStream, line)) {
                            ...
                            size_t rank;
                            turbo::SimpleAtoi(result[0], &rank);
                            dataCount++;
                            if (rank != dataCount) {
                                std::stringstream msg;
        ...
        static void sumup(std::istream &resultStream,
            ...
            while (getline(resultStream, line)) {
                ...
                if (line.size() != 0 && line.at(0) == '#') {
                    ...
                    if (tf.size() >= 1 && tf[0] == "# Query No.") {
                        size_t n;
                        if (tf.size() >= 2 && turbo::SimpleAtoi(tf[1], &n) && n == queryNo) {
                            ...
                            while (getline(resultStream, line)) {
                                ...
                                if (line.size() != 0 && line.at(0) == '#') {
                                    ...
                                    if (gtf.size() >= 2 && (gtf[0] == "# Epsilon" || gtf[0] == "# Factor")) {
                                        ...
                                        turbo::SimpleAtod(gtf[1], &queryTime);
                                    } else if (gtf.size() >= 2 && gtf[0] == "# Distance Computation") {
                                        turbo::SimpleAtoi(gtf[1], &distanceCount);
                                    } else if (gtf.size() >= 2 && gtf[0] == "# Visit Count") {
                                        turbo::SimpleAtoi(gtf[1], &visitCount);
                                        ...
                                        double key;
                                        if (epsilon != "") {
                                            turbo::SimpleAtod(epsilon, &key);
                                            keyValue = "Factor (Epsilon)";
                                        } else if (expansion != "") {
                                            turbo::SimpleAtod(expansion, &key);
                                            keyValue = "Expansion";
                                        } else {
                                            std::stringstream msg;
        ...
        static void adjustSearchEdgeSize(Args &args) {
            ...
            if (opt.size() != 0) {
                std::vector<std::string> tokens = turbo::StrSplit(opt, ":");
                if (tokens.size() >= 1) { turbo::SimpleAtof(tokens[0], &baseAccuracyRange.first); }
                if (tokens.size() >= 2) { turbo::SimpleAtof(tokens[1], &baseAccuracyRange.second); }
                if (tokens.size() >= 3) { turbo::SimpleAtof(tokens[2], &rateAccuracyRange.first); }
                if (tokens.size() >= 4) { turbo::SimpleAtof(tokens[3], &rateAccuracyRange.second); }
            }
        ...
        static void createGroundTruth(tann::Index &index, double epsilon, Command::SearchParameters &searchParameters,
