...
void get_bin_metadata(AlignedFileReader &reader, size_t &npts, size_t &ndim, size_t offset)
{
    ...
    if ((*(ctx.m_pRequestsStatus))[0] == IOContext::READ_SUCCESS)
    {
        npts = buf[0];
        ndim = buf[1];
        tann::cout << "File has: " << npts << " points, " << ndim << " dimensions at offset: " << offset
                      << std::endl;
    }
    else
    {
        ...
        throw tann::ANNException(str.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
    }
}
                                 ...
                                 const size_t &rounded_dim, size_t offset)
{
    if (data == nullptr)
    {
        tann::cerr << "Memory was not allocated for " << data << " before calling the load function. Exiting..."
                      << std::endl;
        throw tann::ANNException("Null pointer passed to copy_aligned_data_from_file()", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }
    ...
    if (ndim != dim || npts != pts)
    {
        ...
        throw tann::ANNException(ss.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    ...
    while (i < npts)
    {
        ...
        for (int k = 0; k < read_requests.size(); k++)
        {
            ...
            {
                throw tann::ANNException("Load data from file using AlignedReader failed.", -1, __FUNCSIG__,
                                            __FILE__, __LINE__);
            }
        }
    }
...
template <typename T> void read_array(AlignedFileReader &reader, T *data, size_t size, size_t offset)
{
    if (data == nullptr)
    {
        throw tann::ANNException("read_array requires an allocated buffer.", -1);
...
template TURBO_DLL void load_bin<uint8_t>(AlignedFileReader &reader, std::unique_ptr<uint8_t[]> &data,
                                                  size_t &npts, size_t &ndim, size_t offset);
template TURBO_DLL void load_bin<int8_t>(AlignedFileReader &reader, std::unique_ptr<int8_t[]> &data,
                                                 size_t &npts, size_t &ndim, size_t offset);
template TURBO_DLL void load_bin<uint32_t>(AlignedFileReader &reader, std::unique_ptr<uint32_t[]> &data,
                                                   size_t &npts, size_t &ndim, size_t offset);
template TURBO_DLL void load_bin<uint64_t>(AlignedFileReader &reader, std::unique_ptr<uint64_t[]> &data,
                                                   size_t &npts, size_t &ndim, size_t offset);
template TURBO_DLL void load_bin<int64_t>(AlignedFileReader &reader, std::unique_ptr<int64_t[]> &data,
                                                  size_t &npts, size_t &ndim, size_t offset);
template TURBO_DLL void load_bin<float>(AlignedFileReader &reader, std::unique_ptr<float[]> &data, size_t &npts,
                                                size_t &ndim, size_t offset);

template TURBO_DLL void load_bin<uint8_t>(AlignedFileReader &reader, uint8_t *&data, size_t &npts, size_t &ndim,
                                                  size_t offset);
template TURBO_DLL void load_bin<int64_t>(AlignedFileReader &reader, int64_t *&data, size_t &npts, size_t &ndim,
                                                  size_t offset);
template TURBO_DLL void load_bin<uint64_t>(AlignedFileReader &reader, uint64_t *&data, size_t &npts,
                                                   size_t &ndim, size_t offset);
template TURBO_DLL void load_bin<uint32_t>(AlignedFileReader &reader, uint32_t *&data, size_t &npts,
                                                   size_t &ndim, size_t offset);
template TURBO_DLL void load_bin<int32_t>(AlignedFileReader &reader, int32_t *&data, size_t &npts, size_t &ndim,
                                                  size_t offset);

template TURBO_DLL void copy_aligned_data_from_file<uint8_t>(AlignedFileReader &reader, uint8_t *&data,
                                                                     size_t &npts, size_t &dim,
                                                                     const size_t &rounded_dim, size_t offset);
template TURBO_DLL void copy_aligned_data_from_file<int8_t>(AlignedFileReader &reader, int8_t *&data,
                                                                    size_t &npts, size_t &dim,
                                                                    const size_t &rounded_dim, size_t offset);
template TURBO_DLL void copy_aligned_data_from_file<float>(AlignedFileReader &reader, float *&data,
                                                                   size_t &npts, size_t &dim, const size_t &rounded_dim,
                                                                   size_t offset);

template TURBO_DLL void read_array<char>(AlignedFileReader &reader, char *data, size_t size, size_t offset);

template TURBO_DLL void read_array<uint8_t>(AlignedFileReader &reader, uint8_t *data, size_t size,
                                                    size_t offset);
template TURBO_DLL void read_array<int8_t>(AlignedFileReader &reader, int8_t *data, size_t size, size_t offset);
template TURBO_DLL void read_array<uint32_t>(AlignedFileReader &reader, uint32_t *data, size_t size,
                                                     size_t offset);
template TURBO_DLL void read_array<float>(AlignedFileReader &reader, float *data, size_t size, size_t offset);

template TURBO_DLL void read_value<uint8_t>(AlignedFileReader &reader, uint8_t &value, size_t offset);
template TURBO_DLL void read_value<int8_t>(AlignedFileReader &reader, int8_t &value, size_t offset);
template TURBO_DLL void read_value<float>(AlignedFileReader &reader, float &value, size_t offset);
template TURBO_DLL void read_value<uint32_t>(AlignedFileReader &reader, uint32_t &value, size_t offset);
template TURBO_DLL void read_value<uint64_t>(AlignedFileReader &reader, uint64_t &value, size_t offset);
