...
namespace tann {
    ...
    size_t calculate_num_pq_chunks(double final_index_ram_limit,
        ...
        TURBO_LOG(INFO) << "Calculated num_pq_chunks :" << num_pq_chunks;
        if (param_list.size() >= 6) {
            ...
            if (compress_ratio > 0 && compress_ratio <= 1) {
                ...
                if (chunks_by_cr > 0 && chunks_by_cr < num_pq_chunks) {
                    TURBO_LOG(INFO) << "Compress ratio:" << compress_ratio
                ...
                TURBO_LOG(ERROR) << "Compression ratio: " << compress_ratio
        ...
        TURBO_LOG(INFO) << "Compressing " << dim << "-dimensional data into "
    ...
    T *generateRandomWarmup(uint64_t warmup_num, uint64_t warmup_dim,
        ...
        T *warmup = nullptr;
        warmup_num = 100000;
        TURBO_LOG(INFO) << "Generating random warmup file with dim " << warmup_dim
    ...
    T *load_warmup(MemoryMappedFiles &files, const std::string &cache_warmup_file,
      ...
      if (files.fileExists(cache_warmup_file)) {
        ...
        TURBO_LOG(INFO) << "In the warmup file: " << cache_warmup_file
                      << " File dim: " << file_dim
    ...
    int merge_shards(const std::string &vamana_prefix,
        ...
        nnodes++;
        TURBO_LOG(INFO) << "# nodes: " << nnodes << ", max. degree: " << max_degree;

        // compute inverse map: node -> shards
        std::vector<std::pair<unsigned, unsigned>> node_shard;
        node_shard.reserve(nelems);
        for (_u64 shard = 0; shard < nshards; shard++) {
            TURBO_LOG(INFO) << "Creating inverse map -- shard #" << shard;
        ...
        TURBO_LOG(INFO) << "Finished computing node -> shards map";
        ...
        TURBO_LOG(INFO) << "Max input width: " << max_input_width
        ...
        for (const auto &id_shard: node_shard) {
            ...
            if (shard_nnbrs == 0) {
                ss << "WARNING: shard #" << shard_id << ", node_id "
                           << node_id << " has 0 nbrs";
    ...
    void breakup_dense_points(const std::string data_file,
        ...
        TURBO_LOG(INFO) << "fraction of dense points with >= " << density
                   << " labels = " << (float) dense_pts / (float) npts;

        if (labels_per_point.size() != 0) {
            TURBO_LOG(INFO) << labels_per_point.size() << " is the new number of points";
        ...
        if (dummy_pt_ids.size() != 0) {
            TURBO_LOG(INFO) << dummy_pt_ids.size()
                       << " is the number of dummy points created";
    ...
    void extract_shard_labels(
        ...
        TURBO_LOG(INFO) << "Extracting labels for shard";
    ...
    int build_merged_vamana_index(
        ...
        if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
            TURBO_LOG(INFO) << "Full index fits in RAM budget, should consume at most "
        ...
        TURBO_LOG(INFO) << timer.elapsed_seconds_for_step("partitioning data");
        ...
        TURBO_LOG(INFO) << timer.elapsed_seconds_for_step(
        ...
        TURBO_LOG(INFO) << timer.elapsed_seconds_for_step("merging indices");
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
    ...
    void create_disk_layout(const std::string base_file,
        ...
        TURBO_LOG(INFO) << "medoid: " << medoid << "B";
        TURBO_LOG(INFO) << "max_node_len: " << max_node_len << "B";
        TURBO_LOG(INFO) << "nnodes_per_sector: " << nnodes_per_sector << "B";
        ...
        for (_u64 sector = 0; sector < n_sectors; sector++) {
            if (sector % 100000 == 0) {
                TURBO_LOG(INFO) << "Sector #" << sector << "written";
            }
        ...
        if (append_reorder_data) {
            ...
            for (_u64 sector = 0; sector < n_reorder_sectors; sector++) {
                if (sector % 100000 == 0) {
                    TURBO_LOG(INFO) << "Reorder data Sector #" << sector << "written";
                }
...
// This code should only be available in the "build" DLL.
    ...
    defined(TANN_BUILD)
        ...
        TURBO_LOG(INFO) << timer.elapsed_seconds_for_step("generating disk layout");
        ...
        auto e = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> diff = e - s;
        TURBO_LOG(INFO) << "Indexing time: " << diff.count();
