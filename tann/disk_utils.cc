...
namespace tann {
  ...
  void add_new_file_to_single_index(std::string index_file,
    ...
    _u64                    nr, nc;
    tann::load_bin<_u64>(index_file, metadata, nr, nc);
    if (nc != 1) {
    ...
    tann::save_bin<_u64>(index_file, new_meta.data(), new_meta.size(), 1);
  }
  ...
  size_t calculate_num_pq_chunks(double final_index_ram_limit,
    ...
    if (param_list.size() >= 6) {
      ...
      if (compress_ratio > 0 && compress_ratio <= 1) {
        ...
        if (chunks_by_cr > 0 && chunks_by_cr < num_pq_chunks) {
          tann::cout << "Compress ratio:" << compress_ratio
        ...
        tann::cerr << "Compression ratio: " << compress_ratio
    ...
    tann::cout << "Compressing " << dim << "-dimensional data into "
  ...
  T *generateRandomWarmup(uint64_t warmup_num, uint64_t warmup_dim,
    ...
    T *warmup = nullptr;
    warmup_num = 100000;
    tann::cout << "Generating random warmup file with dim " << warmup_dim
  ...
  T *load_warmup(MemoryMappedFiles &files, const std::string &cache_warmup_file,
    ...
    if (files.fileExists(cache_warmup_file)) {
      tann::load_aligned_bin<T>(files, cache_warmup_file, warmup, warmup_num,
                                   file_dim, file_aligned_dim);
      tann::cout << "In the warmup file: " << cache_warmup_file
                    << " File dim: " << file_dim
  ...
  T *load_warmup(const std::string &cache_warmup_file, uint64_t &warmup_num,
    ...
    if (file_exists(cache_warmup_file)) {
      tann::load_aligned_bin<T>(cache_warmup_file, warmup, warmup_num,
                                   file_dim, file_aligned_dim);
      if (file_dim != warmup_dim || file_aligned_dim != warmup_aligned_dim) {
        std::stringstream stream;
        stream << "Mismatched dimensions in sample file. file_dim = "
  ...
  int merge_shards(const std::string &vamana_prefix,
    ...
    nnodes++;
    tann::cout << "# nodes: " << nnodes << ", max. degree: " << max_degree
                  << std::endl;
    ...
    tann::cout << "Max input width: " << max_input_width
    ...
    for (const auto &id_shard : node_shard) {
      ...
      if (shard_nnbrs == 0) {
        tann::cout << "WARNING: shard #" << shard_id << ", node_id "
  ...
  void breakup_dense_points(const std::string data_file,
    ...
    tann::cout << "fraction of dense points with >= " << density
                  << " labels = " << (float) dense_pts / (float) npts
                  << std::endl;

    if (labels_per_point.size() != 0) {
      tann::cout << labels_per_point.size() << " is the new number of points"
                    << std::endl;
  ...
  int build_merged_vamana_index(
    ...
    if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
      ...
      std::unique_ptr<tann::Index<T, TagT, LabelT>> _pvamanaIndex =
          std::unique_ptr<tann::Index<T, TagT, LabelT>>(
              new tann::Index<T, TagT, LabelT>(
    ...
    tann::cout << timer.elapsed_seconds_for_step("partitioning data")
                  << std::endl;
    ...
    for (int p = 0; p < num_parts; p++) {
      ...
      std::unique_ptr<tann::Index<T>> _pvamanaIndex =
          std::unique_ptr<tann::Index<T>>(new tann::Index<T>(
      ...
      if (!use_filters) {
        ...
        tann::extract_shard_labels(label_file, shard_ids_file,
                                      shard_labels_file);
        if (universal_label != "") {  //  indicates no universal label
          LabelT unv_label_as_num = 0;
          _pvamanaIndex->set_universal_label(unv_label_as_num);
    ...
    tann::cout << timer.elapsed_seconds_for_step(
                         "building indices on shards")
                  << std::endl;

    timer.reset();
    tann::merge_shards(merged_index_prefix + "_subshard-", "_mem.index",
                          merged_index_prefix + "_subshard-", "_ids_uint32.bin",
                          num_parts, R, mem_index_path, medoids_file,
                          use_filters, labels_to_medoids_file);
    tann::cout << timer.elapsed_seconds_for_step("merging indices")
                  << std::endl;
  ...
  uint32_t optimize_beamwidth(
      std::unique_ptr<tann::PQFlashIndex<T, LabelT>> &pFlashIndex,
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  void create_disk_layout(const std::string base_file,
    ...
    tann::cout << "nnodes_per_sector: " << nnodes_per_sector << "B"
                  << std::endl;
    ...
    if (append_reorder_data) {
      ...
      for (_u64 sector = 0; sector < n_reorder_sectors; sector++) {
        if (sector % 100000 == 0) {
          tann::cout << "Reorder data Sector #" << sector << "written"
                        << std::endl;
        }
    ...
    tann::cout << "Output disk index file written to " << output_file
                  << std::endl;
  }
  ...
  int build_disk_index(const char *dataFilePath, const char *indexFilePath,
    ...
    if (compareMetric == tann::Metric::INNER_PRODUCT) {
      ...
      float max_norm_of_base =
          tann::prepare_base_for_inner_products<T>(base_file, prepped_base);
    ...
    tann::cout << "Starting index build: R=" << R << " L=" << L
    ...
    tann::cout << timer.elapsed_seconds_for_step("generating quantized data")
                  << std::endl;
...
// This code should only be available in the "build" DLL.
    ...
    timer.reset();
    tann::build_merged_vamana_index<T, LabelT>(
        data_file_to_use.c_str(), tann::Metric::L2, L, R, p_val,
    ...
    timer.reset();
    if (!use_disk_pq) {
      tann::create_disk_layout<T>(data_file_to_use.c_str(), mem_index_path,
                                     disk_index_path);
    } else {
      if (!reorder_data)
        tann::create_disk_layout<_u8>(disk_pq_compressed_vectors_path,
    ...
    tann::cout << timer.elapsed_seconds_for_step("generating disk layout")
                  << std::endl;
