...
void WindowsAlignedFileReader::register_thread()
{
    ...
    {
        tann::cout << "Warning:: Duplicate registration for thread_id : " << std::this_thread::get_id() << std::endl;
    }
    ...
    if (ctx.fhandle == INVALID_HANDLE_VALUE)
    {
        std::stringstream ss;
        ss << m_filename.c_str();
        tann::cout << "Error opening " << ss.str() << " -- error=" << GetLastError() << std::endl;
    }
...
IOContext &WindowsAlignedFileReader::get_ctx()
{
    ...
    if (ctx_map.find(std::this_thread::get_id()) == ctx_map.end())
    {
        ...
        throw tann::ANNException(stream.str(), -2, __FUNCSIG__, __FILE__, __LINE__);
    }
...
void WindowsAlignedFileReader::read(std::vector<AlignedRead> &read_reqs, IOContext &ctx, bool async)
{
    ...
    for (uint64_t i = 0; i < n_batches; i++)
    {
        ...
        for (uint64_t j = 0; j < batch_size; j++)
        {
            ...
            {
                tann::cerr << "Error queueing IO -- ReadFile returned TRUE" << std::endl;
            }
        }
        DWORD n_read = 0;
        ...
        while (n_complete < batch_size)
        {
            ...
            else
            {
                ...
                if (lp_os == NULL)
                {
                    ...
                    {
                        tann::cerr << "GetQueuedCompletionStatus() failed "
                                         "with error = "
                                      << error << std::endl;
                        throw tann::ANNException("GetQueuedCompletionStatus failed with error: ", error, __FUNCSIG__,
                                                    __FILE__, __LINE__);
                    }
