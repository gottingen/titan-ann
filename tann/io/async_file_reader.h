...
#ifndef TANN_IO_ASYNC_FILE_READER_H_
#define TANN_IO_ASYNC_FILE_READER_H_

#include "tann/io/disk_io.h"
...
namespace tann {
    namespace Helper {
        void SetThreadAffinity(int threadID, std::thread &thread, NumaStrategy socketStrategy = NumaStrategy::LOCAL,
                               OrderStrategy idStrategy = OrderStrategy::ASC);
        ...
        public:
            ...
            virtual bool Initialize(const char *filePath, int openMode,
                ...
                if (m_fileHandle <= 0) {
                    TLOG_ERROR("Failed to create file handle: {}", filePath);
                    return false;
                }
                ...
                for (int i = 0; i < threadPoolSize; i++) {
                    ...
                    if (ret < 0) {
                        TLOG_ERROR("Cannot setup aio: {}", strerror(errno));
                        return false;
                    }
                }
            ...
            WriteBinary(std::uint64_t writeSize, const char *buffer, std::uint64_t offset = UINT64_MAX) {
                return 0;
            }

            virtual std::uint64_t
            ReadString(std::uint64_t &readSize, std::unique_ptr<char[]> &buffer, char delim = '\n',
                       std::uint64_t offset = UINT64_MAX) {
                return 0;
            }
            ...
            virtual bool ReadFileAsync(AsyncReadRequest &readRequest) {
                ...
                while (curTry < maxTry &&
                       syscall(__NR_io_submit, m_iocps[(readRequest.m_status & 0xffff) % m_iocps.size()], 1, iocbs) <
                       1) {
        ...
        void BatchReadFileAsync(std::vector<std::shared_ptr<Helper::DiskIO>> &handlers, AsyncReadRequest *readRequests,
                                int num);
    }
}

#endif // TANN_IO_ASYNC_FILE_READER_H_
