...
TxtVectorReader::LoadFile(const std::string& p_filePaths)
{
    ...
    for (const auto& fileInfo : files)
    {
        ...
        {
            TLOG_ERROR("File {} not exists or can't access.", fileInfo.first.c_str());
                                ...
                                std::size_t p_fileBlockSize)
{
    ...
    {
        TLOG_ERROR("Unable to open file: {}",p_filePath.c_str());
        return ErrorCode::FailedOpenFile;
    }

    TLOG_INFO("Begin Subtask: {}, start offset position: {}", p_subTaskID, startpos);
    ...
    {
        TLOG_ERROR("Unable to create files: {} {} {}", (m_vectorOutput + subFileSuffix).c_str(), (m_metadataConentOutput + subFileSuffix).c_str(), (m_metadataIndexOutput + subFileSuffix).c_str());
    ...
    while (totalRead <= p_fileBlockSize)
    {
        ...
        {
            TLOG_ERROR("Subtask: {} cannot parsing line:{}", p_subTaskID, currentLine.get());
...
#undef DefineVectorValueType
        ...
        if (!parseSuccess)
        {
            TLOG_ERROR("Subtask: {} cannot parsing vector:{}", p_subTaskID, currentLine.get());
            return ErrorCode::FailedParseValue;
        }
        ...
        if (output->WriteBinary(vectorByteSize, (char*)vector.get()) != vectorByteSize ||
            ...
            TLOG_ERROR("Subtask: {} cannot write line:{}", p_subTaskID, currentLine.get());
            return ErrorCode::DiskIOFail;
        }
        metaOffset += tabIndex;
    }
    if (metaIndex->WriteBinary(sizeof(metaOffset), (const char*)&metaOffset) != sizeof(metaOffset)) {
        TLOG_ERROR("Subtask: {} cannot write final offset!", p_subTaskID);
        return ErrorCode::DiskIOFail;
    }
...
TxtVectorReader::MergeData()
{
    ...
    {
        TLOG_ERROR("Unable to create files: {} {} {}", m_vectorOutput.c_str(), m_metadataConentOutput.c_str(), m_metadataIndexOutput.c_str());
        return ErrorCode::FailedCreateFile;
    }
    ...
    if (output->WriteBinary(sizeof(totalRecordCount), (char*)(&totalRecordCount)) != sizeof(totalRecordCount)) {
        TLOG_ERROR("Unable to write file: {}", m_vectorOutput.c_str());
        return ErrorCode::DiskIOFail;
    }
    ...
    for (std::uint32_t i = 0; i < m_subTaskCount; ++i)
    {
        ...
        {
            TLOG_ERROR("Unable to open file: {}", file.c_str());
            return ErrorCode::FailedOpenFile;
        }
    ...
    for (std::uint32_t i = 0; i < m_subTaskCount; ++i)
    {
        ...
        while ( readSize != 0 ) {
            if (meta->WriteBinary(readSize, bufferHolder.get()) != readSize) {
                TLOG_ERROR("Unable to write file: {}", m_metadataConentOutput.c_str());
                return ErrorCode::DiskIOFail;
            }
            readSize = input->ReadBinary(bufferSize, bufferHolder.get());
        }
    ...
    if (metaIndex->WriteBinary(sizeof(totalRecordCount), (char*)(&totalRecordCount)) != sizeof(totalRecordCount)) {
        TLOG_ERROR("Unable to write file: {}", m_metadataIndexOutput.c_str());
        return ErrorCode::DiskIOFail;
    }
    ...
    for (std::uint32_t i = 0; i < m_subTaskCount; ++i)
    {
        ...
        for (SizeType remains = m_subTaskRecordCount[i]; remains > 0;)
        {
            ...
            if (input->ReadBinary(readBytesCount, buf) != readBytesCount) {
                TLOG_ERROR("Unable to read file: {}", file.c_str());
                return ErrorCode::DiskIOFail;
            }
