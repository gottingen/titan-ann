...
namespace tann {
    ...
    BasicVectorSet::~BasicVectorSet() = default;
    ...
    ErrorCode BasicVectorSet::AppendSave(const std::string &p_vectorFile) const {
        ...
        // Update count header
        {
            auto fp_write = tann::f_createIO();
            if (fp_write == nullptr ||
                !fp_write->Initialize(p_vectorFile.c_str(),
        ...
        {
            auto fp_append = tann::f_createIO();
            if (fp_append == nullptr ||
                !fp_append->Initialize(p_vectorFile.c_str(), std::ios::binary | std::ios::out | std::ios::app))
                return ErrorCode::FailedOpenFile;
...
case tann::VectorValueType::Name: \
tann::COMMON::Utils::BatchNormalize<Type>(reinterpret_cast<Type *>(m_data.Data()), m_vectorCount, m_dimension, tann::COMMON::Utils::GetBase<Type>(), p_threads); \
break; \
