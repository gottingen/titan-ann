...
#include "tann/concurrent/lock_free.h"

namespace tann {
    typedef typename tann::Helper::LockFree::LockFreeVector<std::uint64_t> MetadataOffsets;
    ...
    ErrorCode
    MetadataSet::RefineMetadata(std::vector<SizeType> &indices, std::shared_ptr<Helper::DiskIO> p_metaOut,
        ...
        TLOG_INFO("Save MetaIndex({}) Meta({})", R, offset);
        return ErrorCode::Success;
    }


    ErrorCode
    MetadataSet::RefineMetadata(std::vector<SizeType> &indices, const std::string &p_metaFile,
    ...
    bool
    MetadataSet::GetMetadataOffsets(const std::uint8_t *p_meta, const std::uint64_t p_metaLength,
                                    std::uint64_t *p_offsets,
                                    std::uint64_t p_offsetLength, char p_delimiter) {
        ...
        if (current < p_offsetLength) {
            TLOG_ERROR(
                    "The metadata({}) and vector({}) numbers are not match! Check whether it is unicode encoding issue.",
    ...
    FileMetadataSet::FileMetadataSet(const std::string &p_metafile, const std::string &p_metaindexfile,
        ...
        if (m_fp == nullptr || fpidx == nullptr ||
            !m_fp->Initialize(p_metafile.c_str(), std::ios::binary | std::ios::in) ||
            !fpidx->Initialize(p_metaindexfile.c_str(), std::ios::binary | std::ios::in)) {
            TLOG_ERROR("ERROR: Cannot open meta files {} or {}!", p_metafile.c_str(), p_metaindexfile.c_str());
            throw std::runtime_error("Cannot open meta files");
        }

        if (fpidx->ReadBinary(sizeof(m_count), (char *) &m_count) != sizeof(m_count)) {
            TLOG_ERROR("ERROR: Cannot read FileMetadataSet!");
        ...
        TLOG_INFO("Load MetaIndex({}) Meta({})", m_count, m_offsets[m_count]);
    }
    ...
    ErrorCode
    FileMetadataSet::SaveMetadata(std::shared_ptr<Helper::DiskIO> p_metaOut,
        ...
        IOBINARY(p_metaIndexOut, WriteBinary, sizeof(std::uint64_t) * m_offsets.size(),
                 (const char *) m_offsets.data());
        ...
        TLOG_INFO("Save MetaIndex({}) Meta({})", m_offsets.size() - 1, m_offsets.back());
        return ErrorCode::Success;
    }
    ...
    MemMetadataSet::MemMetadataSet(std::uint64_t p_blockSize, std::uint64_t p_capacity, std::uint64_t p_metaSize)
            : m_count(
            0), m_metadataHolder(ByteArray::c_empty) {
    ...
    MemMetadataSet::MemMetadataSet(std::shared_ptr<Helper::DiskIO> p_metain,
                                   std::shared_ptr<Helper::DiskIO> p_metaindexin,
                                   std::uint64_t p_blockSize, std::uint64_t p_capacity, std::uint64_t p_metaSize) {
        if (Init(p_metain, p_metaindexin, p_blockSize, p_capacity, p_metaSize) != ErrorCode::Success) {
            TLOG_ERROR("ERROR: Cannot read MemMetadataSet!");
    ...
    MemMetadataSet::Add(const ByteArray &data) {
        ...
        if (!m_offsets.push_back(m_offsets.back() + data.Length())) {
            TLOG_ERROR("Insert MetaIndex error! DataCapacity overflow!");
    ...
    ErrorCode
    MemMetadataSet::SaveMetadata(std::shared_ptr<Helper::DiskIO> p_metaOut,
                                 std::shared_ptr<Helper::DiskIO> p_metaIndexOut) {
        auto &m_offsets = *static_cast<MetadataOffsets *>(m_pOffsets.get());
        SizeType count = Count();
        IOBINARY(p_metaIndexOut, WriteBinary, sizeof(SizeType), (const char *) &count);
