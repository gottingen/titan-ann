...
namespace tann {
  ...
  struct PQScratch {
    ...
    float* aligned_dist_scratch =
        nullptr;  // MUST BE AT LEAST tann MAX_DEGREE
    ...
    PQScratch(size_t graph_degree, size_t aligned_dim) {
      ...
      tann::alloc_aligned((void**) &aligned_pqtable_dist_scratch,
                             256 * (_u64) MAX_PQ_CHUNKS * sizeof(float), 256);
      tann::alloc_aligned((void**) &aligned_dist_scratch,
                             (_u64) graph_degree * sizeof(float), 256);
      tann::alloc_aligned((void**) &aligned_query_float,
                             aligned_dim * sizeof(float), 8 * sizeof(float));
      tann::alloc_aligned((void**) &rotated_query,
                             aligned_dim * sizeof(float), 8 * sizeof(float));
  ...
  void generate_disk_quantized_data(
      ...
      const tann::Metric compareMetric, const double p_val,
      size_t& disk_pq_dims);
  ...
  void generate_quantized_data(const std::string     data_file_to_use,
                               ...
                               const tann::Metric compareMetric,
