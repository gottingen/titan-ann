...
namespace tann {
    template<typename T>
    TURBO_DLL void generate_label_indices(path input_data_path, path final_index_path_prefix, label_set all_labels,
                                          unsigned R, unsigned L, float alpha, unsigned num_threads);

    TURBO_DLL load_label_index_return_values load_label_index(path label_index_path,
 ...
 */
    ...
    inline std::vector<uint32_t> loadTags(const std::string &tags_file, const std::string &base_file) {
        ...
        if (tags_enabled) {
            ...
            tann::load_bin<std::uint32_t>(tags_file, tag_data, tag_file_npts, tag_file_ndims);
            if (tag_file_ndims != 1) {
                tann::cerr << "tags file error" << std::endl;
                throw tann::ANNException("tag file error", -1, __FUNCSIG__, __FILE__, __LINE__);
            }
            ...
            if (base_file_npts != tag_file_npts) {
                ...
                throw tann::ANNException("point num in tags file mismatch", -1, __FUNCSIG__, __FILE__, __LINE__);
            }
