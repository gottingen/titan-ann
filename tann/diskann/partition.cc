...
namespace tann {
    ...
    void gen_random_slice(const std::string base_file, const std::string output_prefix, double sampling_rate) {
        ...
        tann::cout << "Loading base " << base_file << ". #points: " << npts_u32 << ". #dim: " << nd_u32 << "."
                   << std::endl;
        ...
        tann::cout << "Wrote " << num_sampled_pts_u32 << " points to sample file: " << output_prefix + "_data.bin"
                   << std::endl;
    }
...
// npts*ndims to return sampled_data of size slice_size*ndims.
    ...
    int shard_data_into_clusters(const std::string data_file, float *pivots, const size_t num_centers, const size_t dim,
        ...
        if (basedim32 != dim) {
            tann::cout << "Error. dimensions dont match for train set and base set" << std::endl;
            return -1;
        }
        ...
        for (size_t block = 0; block < num_blocks; block++) {
            ...
            tann::convert_types<T, float>(block_data_T.get(), block_data_float.get(), cur_blk_size, dim);
...
// each shard, and retrieve the actual vectors on demand.
    ...
    retrieve_shard_data_from_ids(const std::string data_file, std::string idmap_filename, std::string data_filename) {
        ...
        tann::load_bin<uint32_t>(idmap_filename, shard_ids, shard_size, tmp);
        ...
        tann::cout << "Written file with " << num_written << " points" << std::endl;
    ...
    int partition(const std::string data_file, const float sampling_rate, size_t num_parts, size_t max_k_means_reps,
        ...
        tann::cout << "Processing global k-means (kmeans_partitioning Step)" << std::endl;
        ...
        tann::save_bin<float>(output_file.c_str(), pivot_data, (size_t) num_parts, train_dim);
        ...
        while (!fit_in_ram) {
            ...
            for (auto &p: cluster_sizes) {
                ...
                double cur_shard_ram_estimate =
                        tann::estimate_ram_usage(p, (uint32_t) train_dim, sizeof(T), (uint32_t) graph_degree);

                if (cur_shard_ram_estimate > max_ram_usage)
                    max_ram_usage = cur_shard_ram_estimate;
            }
            tann::cout << "With " << num_parts
