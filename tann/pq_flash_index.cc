...
namespace tann {
  ...
  PQFlashIndex<T, LabelT>::PQFlashIndex(
      std::shared_ptr<AlignedFileReader> &fileReader, tann::Metric m)
...
#pragma omp critical
  ...
  void PQFlashIndex<T, LabelT>::load_cache_list(
    ...
    tann::alloc_aligned((void **) &coord_cache_buf,
  ...
  void PQFlashIndex<T, LabelT>::generate_cache_list_from_sample_queries(
    ...
    if (files.fileExists(sample_bin)) {
      tann::load_aligned_bin<T>(files, sample_bin, samples, sample_num,
                                   sample_dim, sample_aligned_dim);
    }
#else
    if (file_exists(sample_bin)) {
      tann::load_aligned_bin<T>(sample_bin, samples, sample_num, sample_dim,
                                   sample_aligned_dim);
    }
#endif
    else {
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  void PQFlashIndex<T, LabelT>::cache_bfs_levels(
    ...
    std::unique_ptr<turbo::flat_hash_set<unsigned>> cur_level, prev_level;
    cur_level = std::make_unique<turbo::flat_hash_set<unsigned>>();
    prev_level = std::make_unique<turbo::flat_hash_set<unsigned>>();
...
#ifdef EXEC_ENV_OLS
    tann::load_bin<_u8>(files, pq_compressed_vectors, this->data, npts_u64,
                           nchunks_u64);
#else
    tann::load_bin<_u8>(pq_compressed_vectors, this->data, npts_u64,
                           nchunks_u64);
#endif
...
// read index metadata
    ...
    if (files.fileExists(medoids_file)) {
      size_t tmp_dim;
      tann::load_bin<uint32_t>(files, medoids_file, medoids, num_medoids,
                                  tmp_dim);
#else
...
#ifdef EXEC_ENV_OLS
        tann::load_aligned_bin<float>(files, centroids_file, centroid_data,
                                         num_centroids, tmp_dim,
                                         aligned_tmp_dim);
#else
        tann::load_aligned_bin<float>(centroids_file, centroid_data,
                                         num_centroids, tmp_dim,
                                         aligned_tmp_dim);
#endif
  ...
  void PQFlashIndex<T, LabelT>::cached_beam_search(
    ...
    auto compute_dists = [this, pq_coord_scratch, pq_dists](const unsigned *ids,
                                                            const _u64 n_ids,
                                                            float *dists_out) {
      tann::aggregate_coords(ids, n_ids, this->data, this->n_chunks,
                                pq_coord_scratch);
      tann::pq_dist_lookup(pq_coord_scratch, n_ids, this->n_chunks, pq_dists,
                              dists_out);
    };
    Timer query_timer, io_timer, cpu_timer;

    turbo::flat_hash_set<_u64>  &visited = query_scratch->visited;
