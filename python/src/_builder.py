...
from . import _tannpy as _native_dap
from ._common import (
    VectorDType,
    _assert,
    _assert_2d,
...
from ._tannpy import defaults


def numpy_to_tann_file(vectors: np.ndarray, file_handler: BinaryIO):
    """
    Utility function that writes a Tann binary vector formatted file to the location of your choosing.
...
def _valid_path_and_dtype(
    ...
    if isinstance(data, np.ndarray):
        ...
        with open(vector_bin_path, "wb") as temp_vector_bin:
            numpy_to_tann_file(data, temp_vector_bin)
...
def build_disk_index(
    ...
    """
    This function will construct a Tann Disk Index and save it to disk.

    If you provide a numpy array, it will save this array to disk in a temp location
    in the format Tann's PQ Flash Index builder requires. This temp folder is deleted upon index creation completion
    or error.

    :param data: Either a ``str`` representing a path to a Tann vector bin file, or a numpy.ndarray,
