...
template <class T> struct DiskIndex
{
    ...
    DiskIndex(const tann::Metric metric, const std::string &index_path_prefix, const uint32_t num_threads,
...
template <class T> struct StaticInMemIndex
{
                     ...
                     const uint32_t num_threads, const uint32_t initial_search_complexity)
    {
        ...
        tann::get_bin_metadata(data_path, npoints, ndims);
...
template <typename T>
void build_disk_index2(const tann::Metric metric, const std::string &data_file_path,
                       ...
                       const uint32_t pq_disk_bytes)
{
    ...
    tann::build_disk_index<T>(data_file_path.c_str(), index_prefix_path.c_str(), params.c_str(), metric);
}

template <typename T, typename TagT = IdT, typename LabelT = filterT>
void build_in_memory_index(const tann::Metric &metric, const std::string &vector_bin_path,
...
{
    tann::IndexWriteParameters index_build_params = tann::IndexWriteParametersBuilder(complexity, graph_degree)
                                                           .with_filter_list_size(filter_complexity)
                                                           .with_alpha(alpha)
    ...
    tann::get_bin_metadata(vector_bin_path, data_num, data_dim);
    tann::Index<T, TagT, LabelT> index(metric, data_dim, data_num, false, use_tags, false, use_pq_build,
...
inline void add_variant(py::module_ &m, const std::string &build_name, const std::string &class_name)
{
    ...
    py::class_<StaticInMemIndex<T>>(m, static_index.c_str())
        .def(py::init([](const tann::Metric metric, const std::string &data_path, const std::string &index_path,
                         const uint32_t num_threads, const uint32_t initial_search_complexity) {
    ...
    py::class_<DynamicInMemIndex<T>>(m, dynamic_index.c_str())
        .def(py::init([](const tann::Metric metric, const size_t dim, const size_t max_points,
             ...
             py::arg("saturate_graph") = tann::defaults::SATURATE_GRAPH,
             py::arg("max_occlusion_size") = tann::defaults::MAX_OCCLUSION_SIZE,
             py::arg("alpha") = tann::defaults::ALPHA, py::arg("num_threads") = tann::defaults::NUM_THREADS,
             py::arg("filter_complexity") = tann::defaults::FILTER_LIST_SIZE,
             py::arg("num_frozen_points") = tann::defaults::NUM_FROZEN_POINTS_DYNAMIC,
    ...
    py::class_<DiskIndex<T>>(m, disk_name.c_str())
        .def(py::init([](const tann::Metric metric, const std::string &index_path_prefix, const uint32_t num_threads,
                         const size_t num_nodes_to_cache, const uint32_t cache_mechanism) {
...
PYBIND11_MODULE(_tannpy, m)
{
    m.doc() = "titan-ann Python Bindings";
#ifdef VERSION_INFO
    ...
    py::module_ default_values = m.def_submodule(
        "defaults",
        "A collection of the default values used for common tann operations. `GRAPH_DEGREE` and `COMPLEXITY` are not"
    ...
    default_values.attr("ALPHA") = tann::defaults::ALPHA;
    default_values.attr("NUM_THREADS") = tann::defaults::NUM_THREADS;
    default_values.attr("MAX_OCCLUSION_SIZE") = tann::defaults::MAX_OCCLUSION_SIZE;
    default_values.attr("FILTER_COMPLEXITY") = tann::defaults::FILTER_LIST_SIZE;
    default_values.attr("NUM_FROZEN_POINTS_STATIC") = tann::defaults::NUM_FROZEN_POINTS_STATIC;
    default_values.attr("NUM_FROZEN_POINTS_DYNAMIC") = tann::defaults::NUM_FROZEN_POINTS_DYNAMIC;
    default_values.attr("SATURATE_GRAPH") = tann::defaults::SATURATE_GRAPH;
    default_values.attr("GRAPH_DEGREE") = tann::defaults::MAX_DEGREE;
    default_values.attr("COMPLEXITY") = tann::defaults::BUILD_LIST_SIZE;
