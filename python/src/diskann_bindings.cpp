...
struct DiskANNIndex {
  ...
  DiskANNIndex(tann::Metric metric) {
#ifdef _WINDOWS
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  auto batch_search_numpy_input(
    ...
    tann::QueryStats *stats = new tann::QueryStats[num_queries];
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    collective_stats["mean_latency"] = tann::get_mean_stats<double>(
        stats, num_queries,
        [](const tann::QueryStats &stats) { return stats.total_us; });
    collective_stats["latency_999"] = tann::get_percentile_stats<double>(
        stats, num_queries, 0.999,
        [](const tann::QueryStats &stats) { return stats.total_us; });
    collective_stats["mean_ssd_ios"] = tann::get_mean_stats<double>(
        stats, num_queries,
        [](const tann::QueryStats &stats) { return stats.n_ios; });
    collective_stats["mean_dist_comps"] = tann::get_mean_stats<double>(
        stats, num_queries,
        [](const tann::QueryStats &stats) { return stats.n_cmps; });
    delete[] stats;
...
PYBIND11_MODULE(diskannpy, m) {
  ...
  py::class_<DiskANNIndex<float>>(m, "DiskANNFloatIndex")
      .def(py::init([](tann::Metric metric) {
      ...
      .def(
          ...
          [](DiskANNIndex<float> &self, const char *data_file_path,
            ...
            tann::build_disk_index<float>(data_file_path, index_prefix_path,
  ...
  py::class_<DiskANNIndex<int8_t>>(m, "DiskANNInt8Index")
      ...
      .def(
          ...
          [](DiskANNIndex<int8_t> &self, const char *data_file_path,
            ...
            if (pq_disk_bytes > 0)
              params = params + " " + std::to_string(pq_disk_bytes);
            tann::build_disk_index<int8_t>(data_file_path, index_prefix_path,
                                              params.c_str(),
                                              self.get_metric());
          },
          py::arg("data_file_path"), py::arg("index_prefix_path"), py::arg("R"),
