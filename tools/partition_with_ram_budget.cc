...
int main(int argc, char **argv) {
    ...
    if (std::string(argv[1]) == std::string("float"))
        tann::partition_with_ram_budget<float>(data_path, sampling_rate, ram_budget, graph_degree, prefix_path, k_index);
    else if (std::string(argv[1]) == std::string("int8"))
        tann::partition_with_ram_budget<int8_t>(data_path, sampling_rate, ram_budget, graph_degree, prefix_path, k_index);
    else if (std::string(argv[1]) == std::string("uint8"))
        tann::partition_with_ram_budget<uint8_t>(data_path, sampling_rate, ram_budget, graph_degree, prefix_path, k_index);
    else
