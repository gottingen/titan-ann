...
int main(int argc, char **argv) {
    ...
    if (std::string(argv[1]) == std::string("float"))
        tann::partition<float>(data_path, sampling_rate, num_partitions, max_reps, prefix_path, k_index);
    else if (std::string(argv[1]) == std::string("int8"))
        tann::partition<int8_t>(data_path, sampling_rate, num_partitions, max_reps, prefix_path, k_index);
    else if (std::string(argv[1]) == std::string("uint8"))
        tann::partition<uint8_t>(data_path, sampling_rate, num_partitions, max_reps, prefix_path, k_index);
    else
