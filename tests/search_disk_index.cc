...
int search_disk_index(
    tann::Metric& metric, const std::string& index_path_prefix,
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    tann::convert_types<uint64_t, uint32_t>(query_result_ids_64.data(),
    ...
    auto mean_cpuus = tann::get_mean_stats<float>(
        stats, query_num,
        [](const tann::QueryStats& stats) { return stats.cpu_us; });

    float recall = 0;
    if (calc_recall_flag) {
      recall = tann::calculate_recall(query_num, gt_ids, gt_dists, gt_dim,
                                         query_result_ids[test_id].data(),
                                         recall_at, recall_at);
  ...
  for (auto L : Lvec) {
    ...
    tann::save_bin<_u32>(cur_result_path, query_result_ids[test_id].data(),
                            query_num, recall_at);

    cur_result_path =
        result_output_prefix + "_" + std::to_string(L) + "_dists_float.bin";
    tann::save_bin<float>(cur_result_path,
