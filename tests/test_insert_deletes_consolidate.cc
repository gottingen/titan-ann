...
void insert_till_next_checkpoint(tann::Index<T, TagT>& index, size_t start,
...
template<typename T, typename TagT>
void delete_from_beginning(tann::Index<T, TagT>& index,
...
void build_incremental_index(
  ...
  if (points_to_skip > num_points) {
    throw tann::ANNException("Asked to skip more points than in data file",
  ...
  tann::Index<T, TagT> index(tann::L2, dim, max_points_to_insert, true,
