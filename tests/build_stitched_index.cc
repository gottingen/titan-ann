...
typedef std::tuple<std::vector<label_set>, turbo::flat_hash_map<std::string, _u32>,
                   label_set>
...
parse_label_file_return_values parse_label_file(path        label_data_path,
  ...
  turbo::flat_hash_map<std::string, _u32> labels_to_number_of_points;
 ...
 */
template<typename T>
turbo::flat_hash_map<std::string, std::vector<_u32>>
generate_label_specific_vector_files(
  ...
  turbo::flat_hash_map<std::string, _u32>              label_to_curr_iovec;
  turbo::flat_hash_map<std::string, std::vector<_u32>> label_id_to_orig_id;
...
generate_label_specific_vector_files_compat(
  ...
  turbo::flat_hash_map<std::string, _u32>              labels_to_curr_vector;
  turbo::flat_hash_map<std::string, std::vector<_u32>> label_id_to_orig_id;
...
void generate_label_indices(path input_data_path, path final_index_path_prefix,
  ...
  tann::Parameters label_index_build_parameters;
  ...
  for (const auto &lbl : all_labels) {
    ...
    tann::get_bin_metadata(curr_label_input_data_path,
                              number_of_label_points, dimension);
    tann::Index<T> index(tann::Metric::L2, dimension,
...
void save_full_index(path final_index_path_prefix, path input_data_path,
                     ...
                     turbo::flat_hash_map<std::string, _u32> entry_points,
...
stitch_indices_return_values stitch_label_indices(
    ...
    label_set                                      all_labels,
    turbo::flat_hash_map<std::string, _u32>              labels_to_number_of_points,
    turbo::flat_hash_map<std::string, _u32>             &label_entry_points,
    turbo::flat_hash_map<std::string, std::vector<_u32>> label_id_to_orig_id_map) {
