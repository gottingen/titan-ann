...
int search_memory_index(tann::Metric &metric, const std::string &index_path,
    ...
    using TagT = uint32_t;
    tann::Index<T, TagT, LabelT> index(metric, query_dim, 0, dynamic, tags);
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        if (calc_recall_flag) {
            ...
            for (unsigned curr_recall = first_recall; curr_recall <= recall_at;
                 curr_recall++) {
                recalls.push_back(tann::calculate_recall(
