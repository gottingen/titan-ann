...
void insert_next_batch(tann::Index<T, TagT, LabelT>& index, size_t start,
...
template<typename T, typename TagT, typename LabelT>
void delete_and_consolidate(tann::Index<T, TagT, LabelT>& index,
                            tann::Parameters& delete_params, size_t start,
                            size_t end) {
  try {
    std::cout << std::endl
    ...
    while (report._status !=
           tann::consolidation_report::status_code::SUCCESS) {
      int wait_time = 5;
      if (report._status ==
...
void build_incremental_index(const std::string& data_path, const unsigned L,
  ...
  if (num_points < max_points_to_insert)
    throw tann::ANNException("num_points < max_points_to_insert", -1,
  ...
  if (consolidate_interval < max_points_to_insert / 1000)
    throw tann::ANNException("ERROR: consolidate_interval is too small", -1,
                                __FUNCSIG__, __FILE__, __LINE__);
  ...
  tann::Index<T, TagT, LabelT> index(
      tann::L2, dim, active_window + 4 * consolidate_interval, true, params,
