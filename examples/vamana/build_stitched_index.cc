 ...
 */
int handle_args(int argc, char **argv, std::string &data_type, path &input_data_path, path &final_index_path_prefix,
    ...
    app.add_option("-P, --data_path", input_data_path, "Input data file in bin format")->required();
    app.add_option("-x, --index_path_prefix", final_index_path_prefix,
    ...
    app.add_option("--stitched_R", stitched_R,
                   "Degree to prune final graph down to")->default_val(100);
    ...
    app.add_option("--label_file", label_data_path,
                   "Input label file in txt format if present")->default_val("");
...
void prune_and_save(path final_index_path_prefix, path full_index_path_prefix, path input_data_path,
    ...
    tann::get_bin_metadata(input_data_path, number_of_label_points, dimension);
    tann::Index<T> index(tann::Metric::L2, dimension, number_of_label_points, false, false);
    ...
    tann::cout.rdbuf(tann_cout_buffer);
...
#else
    if (data_type == "uint8")
        label_id_to_orig_id_map = tann::generate_label_specific_vector_files_compat<uint8_t>(
            input_data_path, labels_to_number_of_points, point_ids_to_labels, all_labels);
    else if (data_type == "int8")
        label_id_to_orig_id_map = tann::generate_label_specific_vector_files_compat<int8_t>(
            input_data_path, labels_to_number_of_points, point_ids_to_labels, all_labels);
    else if (data_type == "float")
        label_id_to_orig_id_map = tann::generate_label_specific_vector_files_compat<float>(
            input_data_path, labels_to_number_of_points, point_ids_to_labels, all_labels);
    else
        throw;
#endif
