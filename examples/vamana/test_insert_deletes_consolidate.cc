...
void insert_till_next_checkpoint(tann::Index<T, TagT> &index, size_t start, size_t end, int32_t thread_count,
...
template<typename T, typename TagT>
void delete_from_beginning(tann::Index<T, TagT> &index, tann::IndexWriteParameters &delete_params,
...
void build_incremental_index(const std::string &data_path, const uint32_t L, const uint32_t R, const float alpha,
    ...
    if (points_to_skip > num_points) {
        throw tann::ANNException("Asked to skip more points than in data file", -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    ...
    tann::Index<T, TagT> index(tann::L2, dim, max_points_to_insert, true, params, L, thread_count, enable_tags,
                               concurrent);
    ...
    T *data = nullptr;
    tann::alloc_aligned(
            (void **) &data, std::max(points_per_checkpoint, beginning_index_size) * aligned_dim * sizeof(T),
            8 * sizeof(T));
    ...
    if (concurrent) {
        ...
        for (size_t start = current_point_offset; start < last_point_threshold;
            ...
            auto insert_task = std::async(std::launch::async, [&]() {
                ...
                insert_till_next_checkpoint(index, start, end, sub_threads, data, aligned_dim);
            });
            insert_task.wait();
            ...
            if (!delete_launched && end >= start_deletes_after &&
                ...
                tann::IndexWriteParameters delete_params =
                        tann::IndexWriteParametersBuilder(params).with_num_threads(sub_threads).build();
...
int main(int argc, char **argv) {
    ...
    app.add_option("--points_to_skip", points_to_skip,
                   "Skip these first set of points from file")->required();
    app.add_option("--max_points_to_insert", max_points_to_insert,
                   "These number of points from the file are inserted after "
                   "points_to_skip")->default_val(0);
    app.add_option("--beginning_index_size", beginning_index_size,
                   "Batch build will be called on these set of points")->required();
    app.add_option("--points_per_checkpoint", points_per_checkpoint,
                   "Insertions are done in batches of points_per_checkpoint")->required();
    app.add_option("--checkpoints_per_snapshot", checkpoints_per_snapshot,
                   "Save the index to disk every few checkpoints")->required();
    app.add_option("--points_to_delete_from_beginning",
                   points_to_delete_from_beginning, "")->required();
    app.add_option("--do_concurrent", concurrent, "")->default_val(false);
    app.add_option("--start_deletes_after", start_deletes_after, "")->default_val(0);
    app.add_option("--start_point_norm", start_point_norm,
                   "Set the start point to a random point on a sphere of this radius")->default_val(0);
    app.add_option(
            "--num_start_points",
            num_start_pts,
