...
class CMakeBuild(build_ext):
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        subprocess.run(
            ["cmake", "-DPYBIND=True", "-S", ext.sourcedir] + cmake_args, cwd=build_temp, check=True
        )
    ...
    def run(self):
        ...
        if windows_build_output_dir.exists():
            ...
            for lib in libs:
                shutil.move(
                    lib,
                    os.path.join(self.build_dir, 'tannpy', os.path.basename(lib))
                )
...
setup(
    ext_modules=[CMakeExtension("tannpy._tannpy", ".")],
    cmdclass={
        "build_ext": CMakeBuild,
        'install_lib': InstallCMakeLibs
    },
    zip_safe=False,
    package_dir={"tannpy": "python/src"},
    exclude_package_data={"tannpy": ["tann_bindings.cc"]}
)
